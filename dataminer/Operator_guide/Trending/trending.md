...
*The trending UI in DataMiner 10.6.5*
...
Based on the available trend data, DataMiner Analytics provides features such as [trend prediction](xref:Trend_prediction), [anomaly detection](xref:Behavioral_anomaly_detection), [pattern matching](xref:Pattern_matching), and [relation learning](xref:Relation_learning).
