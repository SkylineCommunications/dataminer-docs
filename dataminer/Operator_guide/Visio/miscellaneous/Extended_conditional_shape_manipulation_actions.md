...
> - Prior to DataMiner 10.6.0/10.6.2, if the now obsolete Ticketing module is available, the *#Tickets* placeholder can be used to create a condition based on the number of tickets, optionally followed by the domain name in case you only want tickets from a particular domain to be taken into account. This is possible for views, services, and elements. For example: *#Tickets:Internal*.
