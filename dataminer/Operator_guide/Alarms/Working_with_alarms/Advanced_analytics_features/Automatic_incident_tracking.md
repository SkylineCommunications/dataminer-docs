...
> - When an element is swarmed to another Agent while automatic incident tracking is enabled, any active anomalies that include that element will be cleared. However, note that if automatic incident tracking is not enabled but alarms are [grouped manually](#manually-updating-an-alarm-group), swarming an element will only cause such groups to be cleared if a DataMiner version prior to 10.5.0 [CU8]/10.5.11 is used.<!-- RN 43739 -->
