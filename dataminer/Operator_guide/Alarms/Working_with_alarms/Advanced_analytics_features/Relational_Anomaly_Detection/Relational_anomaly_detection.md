...
- RAD is only able to monitor parameters on the local DataMiner Agent. This means that all parameter instances configured using the RAD Manager, the RAD API, or (in versions prior to DataMiner 10.5.9/10.6.0<!--RN 43320-->) the *RelationalAnomalyDetection.xml* file on a given DMA must be hosted on that same DMA. Currently, RAD is not able to simultaneously monitor parameters hosted on different DMAs.
