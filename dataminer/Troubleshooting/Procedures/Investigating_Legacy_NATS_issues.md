...
- **nats-server.log ends with `[FTL] STREAM: Failed to start: nats: Authorization Violation`**
  ...
  Most likely, the `C:\Skyline DataMiner\NATS\nsc` folder is missing or not in sync between DataMiner Agents. You can try to resolve this using the following steps:
  ...
  1. From the primary NAS, copy the entire `C:\Skyline DataMiner\NATS\nsc` folder over to the Agent where NATS fails to start.

     > [!IMPORTANT]
     > Do not zip the folder using the built-in Windows zipping feature, as this can cause files to be lost. Instead copy the folder as-is or use a more robust zipping tool like 7-zip.

  1. Start the NAS service.

  1. Start the NATS service.

  If the error still occurs at this point, this means the currently used credentials were generated by another Agent. See [Remaining steps](#remaining-steps) to fully reset the NATS cluster.
...
This is the main NATS configuration file, located in `C:\Skyline DataMiner\NATS\nats-streaming-server\nats-server.config`.
> [!NOTE]
