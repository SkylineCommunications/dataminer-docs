---
uid: SLScripting_SLProtocol_Reoccurring_Crashes
---

# Investigating reoccurring SLScripting/SLProtocol crashes
...
This allows automated recovery of an SLScripting/SLProtocol crash, without leaving the elements in a possible incorrect state.
However, due to this mechanism it is possible that 1 faulty element causes all other elements in the same SLScripting/SLProtocol process to be restarted frequently.
...
2. Scroll to the bottom, and find the entries for the elements that are being restarted repeatedly due to SLScripting crashes. These are indicated by the `SLScriptingCrashRestart`/`SLProtocolCrashRestart` value in the restart reason column (if everything is OK, `NormalStart` would be shown instead).
