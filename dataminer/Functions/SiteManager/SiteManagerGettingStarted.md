...
For a **self-managed DataMiner System**, the SiteManager DxM must run on the same machine as the DataMiner Agent from which you want to connect to remote data sources. It is supported on DataMiner version 10.5.10/10.6.0 or higher and requires Windows 10 or Windows Server 2019 (build 17134) or later. You can [deploy it from the Admin app](xref:Managing_cloud-connected_nodes#deploying-a-dxm-on-a-dms-node).
