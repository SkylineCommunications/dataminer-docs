...
This feature is available from DataMiner 10.3.6/10.4.0 onwards on DataMiner Systems using [Storage as a Service](xref:STaaS) or a self-managed storage setup with [indexing database](xref:Indexing_Database). In DataMiner 10.3.5, it is available in preview if the soft-launch option *UserDefinableAPI* is enabled. See [soft-launch options](xref:SoftLaunchOptions).
