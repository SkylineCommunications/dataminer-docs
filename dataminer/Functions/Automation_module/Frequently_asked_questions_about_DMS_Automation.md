---
uid: Frequently_asked_questions_about_DMS_Automation
---

# Frequently asked questions about DataMiner Automation

- [How can I execute an Automation Script on an element in timeout?](xref:How_can_I_execute_an_Automation_Script_on_an_element_in_timeout)

- [How do I abort a running Automation script?](xref:How_do_I_abort_a_running_Automation_script)

- [How do I retrieve the current value of a script parameter?](xref:How_do_I_retrieve_the_current_value_of_a_script_parameter)

- [How do I connect to a CMDB from an Automation script?](xref:How_do_I_connect_to_a_CMDB_from_an_Automation_script)

- [How do I parse Correlation Alarm Info data?](xref:How_do_I_parse_Correlation_Alarm_Info_data)

- [How can I manage permanent memory files?](xref:How_can_I_manage_permanent_memory_files)
