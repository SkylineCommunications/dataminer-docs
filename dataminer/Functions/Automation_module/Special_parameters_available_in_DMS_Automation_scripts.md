...
| \<Correlation State> | 65002 | Whether the correlation rule was triggered because the trigger condition matched the alarm ("up"), the base alarms were updated ("update"), or the trigger condition no longer matched the alarm ("down") (only if the script was executed via a correlation rule).<br> Values:<br>- up<br>- update<br>- down |
