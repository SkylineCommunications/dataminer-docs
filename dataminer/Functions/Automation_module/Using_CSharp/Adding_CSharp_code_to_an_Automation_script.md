...
You can interact with the Automation Engine through the `IEngine` interface. A project-based script should use the synchronous `Run(IEngine engine)` entry point. In a legacy inline block, DataMiner may generate a wrapper that receives the concrete `Engine` class.
...
- For small automation scripts, you can enter the logic to be executed without wrapping your code in a class and method.
    ...
    public class Script
    {
     public static void Run(Engine engine) { // Legacy inline C# block wrapper generated by DataMiner.
     engine.GenerateInformation("Hello");
    }
    }
    ```
