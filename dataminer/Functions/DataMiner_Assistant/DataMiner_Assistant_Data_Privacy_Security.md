...
The DataMiner Assistant handles your data with privacy and security as top priorities. This page outlines how data is protected, retained, and managed within the agents from the DataMiner Assistant, including details on the underlying AI infrastructure and the controls in place to keep your data isolated and secure.

## AI infrastructure and data residency

The DataMiner Assistant is powered by OpenAI models hosted on **Microsoft Azure** (Azure OpenAI Service), not directly by OpenAI. This distinction is important for data privacy:

- Azure OpenAI operates under Microsoft's enterprise security and compliance framework, separate from OpenAI's consumer services.
- Models are deployed in specific Azure regions to ensure data remains within a defined **geographical zone**. Data does not leave the region in which it is processed.
  ...
  > [!NOTE]
  > Currently, all AI traffic is processed in the **West Europe** Azure region (Amsterdam) and will not leave that region. For EU/EEA customers, this means data never leaves the European Economic Area, eliminating the need for cross-border data transfer safeguards under GDPR Chapter V (e.g. Standard Contractual Clauses or adequacy decisions). Support for additional Azure regions is planned, allowing customers in other parts of the world to benefit from equivalent data residency guarantees within their own region.
...
Additionally, data at rest is encrypted using Azure-managed encryption keys, ensuring protection even when data is stored.

## Data retention

The Azure OpenAI models used by the DataMiner Assistant are **stateless**: no prompts or completions are stored within the model itself during inferencing. However, Microsoft Azure does apply the following data handling practices:

- **Inferencing (prompts and responses)**: Prompts and generated responses are processed in real time and are not stored in the model. They are not used to train, retrain, or improve base models.
- **Abuse monitoring**: By default, Azure retains a sample of prompts and completions temporarily for abuse monitoring purposes, to detect use that may violate Microsoft's terms of service. The data is retained only for the duration necessary to complete the abuse monitoring process; Microsoft does not publicly specify an exact retention period. This data is stored within the same Azure geography as the deployed resource (West Europe at this time for DataMiner Assistant). It is logically isolated per customer resource and can only be accessed by authorized Microsoft personnel in the event of a flagged violation. For deployments in the European Economic Area, those authorized reviewers are also located within the EEA.
<!-- TODO: Verify whether multi-turn conversation / message history storage is implemented in the DataMiner Assistant -->
- **Conversation context**: If multi-turn conversation features are used, message history may be stored for the duration of the interaction to maintain context. This data is stored at rest in the Skyline Azure tenant within the West Europe region, encrypted using AES-256 encryption, and can be deleted at any time.
...
<!-- what data is kept at DataMiner side + is this configurable? -->
...
- **Insights tab actions**: In the Insights tab, the Assistant can only analyze and provide insights on elements, alarms, and system data that your user role permits you to view. Your permissions directly determine which data the Assistant can access when analyzing your system.
- **Documentation tab actions**: Access to the Documentation tab for retrieving information from the DataMiner documentation is typically available to all authenticated users.
...
**Your data is never used to train or improve AI models.** Azure OpenAI Service provides enterprise data isolation controls that guarantee this:

- The prompts (inputs) and the model's outputs are **not used to train or improve foundation models** unless you explicitly authorized on Azure, which is not the case for the DataMiner Assistant.
...
This approach ensures that your proprietary system information, business logic, and operational data remain completely isolated from any model training processes, by both Skyline and Microsoft.

## Compliance and standards

The DataMiner Assistant adheres to industry-leading security and privacy standards through its use of Azure OpenAI Service:
...
2. Grounding in trusted context and data:
