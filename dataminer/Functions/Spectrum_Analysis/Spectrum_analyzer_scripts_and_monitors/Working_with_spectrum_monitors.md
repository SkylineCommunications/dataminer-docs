...
- In the *monitors* tab, click either ![](~/dataminer/images/monitoradd_32.png) *New monitor* or ![](~/dataminer/images/monitoredit_32.png) *Edit monitor*.
...
> - Settings regarding storage and cleaning of trending data can be configured in the *Trending.TimeSpanSpectrumRecords* tag of the *MaintenanceSettings.xml* file. If [Swarming](xref:Swarming) is not enabled, it is also possible to override the setting per element using an *\<Element>\<Trending>\<TimeSpanSpectrumRecords>* tag in the file *Element.xml*.
...
1. Also in the details section, to create a service matching the monitor, select *Create service*. The generated service will have the same name as the monitor, and will contain all the monitored parameters.
   ...
   > - Spectrum monitors for which a service has been created are indicated with a special icon in the Surveyor, instead of the regular service icon: ![icon](~/dataminer/images/spectrum_monitor.png)
...
1. Click ![priority over monitors](~/dataminer/images/prio_over_monitors_32.png) *Take Priority Over Monitors*.
...
1. In the message box, click *OK*.
   ...
   In the upper-right corner of the spectrum graph, a notice will indicate that you are in prioritized mode.
