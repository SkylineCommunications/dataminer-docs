...
When configuring a `FieldDescriptor` to accommodate multiple values, adjust the type property to match the list variant of the underlying base type. For instance, change `Guid` to `List<Guid>`. From DataMiner 10.6.9/10.7.0 onwards<!--RN 46051-->, `FieldDescriptors` also support `List<string>`. When assigning values to a `DomInstance`, utilize the `AddOrUpdateListFieldValue` extension method to easily add a list of values to the instance.
