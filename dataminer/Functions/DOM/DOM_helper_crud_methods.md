...
> - Single-character `Contains` or `NotContains` filters (e.g., `Name.Contains("$")`) cannot be evaluated by the indexing database, as it expects at least one or two words. Avoid such filters where possible.
