...
1. Select the definition for which you want to configure access.
   ...
   A table appears showing the groups that have access to the selected definition. By default, the table is empty because no groups have access yet.

1. In the upper-right corner, click *+ Add groups* and select the groups that should have access to the definition.

   The selected groups are added to the table with *Full* access by default. This means group members can read, update, and delete the definition and all associated DOM instances.

   ![Example: Groups with access to "Property Values" definition](~/dataminer/images/DOM_Security.png)<br>*Configuring DOM security in DataMiner 10.6.9*

1. Optionally, to limit a group's access to specific DOM instances:

   1. In the *Condition* column, click *Add condition*.

   1. Select a field descriptor.

   1. Specify the value(s) the field must contain.

   When a condition is configured, the group's access level automatically changes from *Full* to *Limited*.

   With *Limited* access, the group can only access DOM instances that contain the specified values in the selected `DomInstanceFieldDescriptor` fields.

1. Optionally, enable the *Read-only* toggle if group members should only be able to view the definition and the DOM instances they can access.
