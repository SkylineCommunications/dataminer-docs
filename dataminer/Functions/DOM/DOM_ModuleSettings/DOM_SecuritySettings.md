...
| FieldValueReferences | List&lt;FieldValueReference&gt; | Stores the references to a `DomDefinition` and `FieldDescriptor` alongside a collection of values that determine what instances will be accessible to the user. A reference is resolved via the IDs of the definition and field descriptor. Supported from DataMiner 10.6.0/10.6.3 onwards.<!-- RN 44233 --> |
...
Additionally, from 10.6.6/10.7.0 onwards<!-- RN 45275 -->, it is also possible to mark such reference as read-only by setting the `ReadOnly` property to `true`. This way, the group will only have read access to all instances of the referenced `DomDefinition`.
...
Additionally, from 10.6.6/10.7.0 onwards<!-- RN 45275 -->, it is also possible to mark such reference as read-only by setting the `ReadOnly` property to `true`. This way, the group will only have read access to the defined subset of instances.
...
When reading or counting, the security filtering is done entirely in the database, which allows this system to be applied on DOM definitions that have millions of DOM instances linked. To ensure optimal performance and stability, some limitations are in place:

- You can define up to 10 values on a reference.
- You can only specify one reference per group for a DOM definition.
- When a user is part of multiple groups, and multiple of those groups have this kind of reference for a certain DOM definition, the reference of the group that comes first alphabetically (case insensitive) will be used. However, we recommend avoiding such situations by defining and using the groups in such a way that overlaps are avoided. Specific groups could be created for this.
...
> - The ID of field descriptor IDs should be unique within a DOM module. If there are duplicate IDs used across multiple section definitions in a module, this may result in unexpected behavior with this type of security reference.

> [!IMPORTANT]
> When `FieldValueReferences` are used to limit access to the instances of a definition, the filter used to read or count the instances **must ideally not exceed 100 OR clauses**. In Elasticsearch and OpenSearch, there is a default limit of 1024 OR clauses. When you specify a reference with three values, this means that the maximum number of conditions sent via the DOM helper read or count method should be three times less than this maximum (341). Since there is a limit of 10 condition values, the recommended max clauses of 100 should prevent queries from failing. With [STaaS](xref:STaaS) there is no such hard limit; however, there is a limit on the general query request size. To ensure a high-performing, stable solution, a maximum of 100 clauses is also recommended here.

When the `ModuleSettings` object is saved with these kinds of references defined, some validation is done to ensure they are valid. If one of these validation rules fail, the `TraceData` will contain a `DomManagerSettingsErrorData` error object that can have one of the following reasons:
...
- The values defined in the `FieldValueReference` will also be checked on validity before they are used to apply the security check. Any invalid values will be ignored. However, these do count towards the maximum number of values mentioned above.
...
#### Count and read requests with DOM definition-scoped filter
...
#### Count and read requests with DOM instance-scoped filter
