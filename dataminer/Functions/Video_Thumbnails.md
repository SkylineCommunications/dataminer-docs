...
You can configure a video thumbnail in two main places: in Visual Overview (Visio shapes rendered in DataMiner Cube) and in the web component (Dashboards and Low‑Code Apps). Both approaches rely on the same underlying mechanism: linking a shape or component to the VideoThumbnails webpage and specifying the required video type, source URL, and optional parameters.

### [Visual Studio](#tab/image-Visual-Studio)
...
### [Web component](#tab/image-web-component)

1. Go to the *Settings* pane.

1. In the *URL* box, enter a value similar to:
...
Below, you find an overview of all supported video formats and what value enter for the video type.
...
- In DataMiner Cube:
  ...
  - *Video Source*: Enter the URL of the video, e.g. `https://videoserver/video.mp4&loop=true`

- In Visio/web component: Use `type=HTML5`, e.g. `#https://dma.local/VideoThumbnails/Video.htm?type=HTML5&source=https://videoserver/video.mp4&loop=true`.
...
- In DataMiner Cube:
  ...
  - *Video Source*: Enter the URL of the image, e.g. `https://<video server>.<stream>.m3u8`.

- In Visio/web component: Use `type=HTML5-HLS`, e.g. `#https://<DMAIP>/VideoThumbnails/Video.htm?type=HTML5-HLS&source=https://<video server>.<stream>.m3u8`
...
- In DataMiner Cube:
  ...
  - *Image Source*: Enter the URL of the image, e.g. `http://IpAddress/Folder/Picture.png`.

  - *Refresh rate*: Specify how frequently you want the image to be refreshed (in milliseconds).

- In Visio/web component: Use `type=Generic Images` with an optional refresh interval, e.g. `#http://DmaIpAddress/VideoThumbnails/Video.htm?type=Generic Images&source=http://IpAddress/Folder/Picture.png&refresh=5000`
...
- In DataMiner Cube:
  ...
  - *Video Source*: Enter the URL of the video, e.g. `http://<DMAIP>/myvideo.mpeg`.
...
- In Visio/web component: Use `type=Generic VLC`, e.g. `#http://<DMAIP>/VideoThumbnails/Video.htm?type=Generic VLC&source=http://<DMAIP>/myvideo.mpeg`
...
| Name | Parameters | Description | Type(s) | Example |
|--|--|--|--|--|
| Referer HTTP header |`referer=` | The VLC component of video thumbnails sends a referer HTTP header when requesting the source URL. The referer URL is by default the URL of the DMA, but you can change it by specifying the parameter "referer=" in the URL of the video thumbnail. | VLC | `#http://localhost/VideoThumbnails/video.htm?type=VLC&source=http%3A%2F%2Fclips.vorwaerts-gmbh.de%2Fbig_buck_bunny.mp4&referer=http%3A%2F%2Fsome%2Freferer%2F.` |
| Password and username | `user=` and `password=` | You can add a password and username in the URL for basic HTTP authentication. However, in that case we advise to always use HTTPS, as otherwise the username and password will not be encrypted. | All | `#https://dma/VideoThumbnails/Video.htm?type=Generic%20Images&source=http%3A%2F%2F10.0.20.101%2Fimages%2Fthumbs%2F4.jpg&user=admin&password=test&refresh=1000` |
| HTTP authorization header | `auth=` | Specify an HTTP authorization header that will be added to the HTTP request when a thumbnail image is requested from the video server. This option is required when the video server expects an authentication token (for example *OAuth2*). | All | `#https://dma/videothumbnails/video.htm?type=Generic%20Images&source=https%3A%2F%2F77.158.55.113%2Fvos-api%2Fmonitor%2Fv1%2Fservices%2F55002da8-37fd-43de-82a9-f6b75089d8c9%2Fthumbnail&auth=bearer%20580a4efa-0aab-4882-af91-7b0118c67f5d` |
| Volume | `volume=` | Available from DataMiner 10.2.0 [CU1]/10.2.4 onwards. Specify the volume for the VLC player in the URL. The volume should be specified as a percentage, ranging from 0 (i.e. muted) to 100. | VLC | `#https://dma.local/VideoThumbnails/Video.htm?type=VLC&source=https://videoserver/video.mp4&volume=50` |
| Loop | `loop=` | Available from DataMiner 10.2.0 [CU1]/10.2.4 onwards. Specify that the video should play continuously in a loop. Set to `false` by default. | All | `#https://dma.local/VideoThumbnails/Video.htm?type=HTML5&source=https://videoserver/video.mp4&loop=true` |
| fitMode | `fitmode=` | Available from DataMiner 10.4.0 [CU18]/10.5.0 [CU6]/10.5.9 onwards<!--RN 43388-->. Specify how the image in a video thumbnail should be displayed. The possible values are:<br>- `fill`: The image will completely cover the container. It may crop parts of the image, but it ensures that there is no empty space. (*default*)<br>- `fit`: The image will be fully visible inside the container while maintaining aspect ratio. There may be empty space if aspect ratios differ.<br>- `stretch`: The image will stretch to exactly fill the container, ignoring aspect ratio. This may cause distortion.<br>- `center`: The image will retain its original size and will be aligned at the center. It may overflow or be cropped.<br>- `shrink`: The image will behave like `fill` or `center`, whichever results in a smaller image. It will only scale down if needed. | All | `#https://dma.local/VideoThumbnails/Video.htm?type=HTML5&source=https://videoserver/video.mp4&loop=true&fitMode=center` |
| VLC toolbar |`toolbar=false`/`showtoolbar=false` | By default the VLC toolbar is shown. To hide the toolbar, specify "toolbar=false" or "showtoolbar=false" in the URL. | VLC | `#http://<DMAIP>/VideoThumbnails/Video.htm?type=Generic VLC&source=http://<DMAIP>/myvideo.mpeg&showtoolbar=false` |
| Refresh rate | `refresh=` | Specify how frequently you want the image to be refreshed (in milliseconds). | Generic Images | `#http://DmaIpAddress/VideoThumbnails/Video.htm?type=Generic Images&source=http://IpAddress/Folder/Picture.png&refresh=5000` |
| Image proxy override | `proxy=` | Generic Images | By default, a thumbnail of type `Generic Images` always uses the DMA as a proxy. However, you can add an extra URL parameter, "proxy", to override this behavior. | `http://<DMA IP>/VideoThumbnails/video.htm?type=Generic%20Images&source=<IMG URL>&proxy=false` |
