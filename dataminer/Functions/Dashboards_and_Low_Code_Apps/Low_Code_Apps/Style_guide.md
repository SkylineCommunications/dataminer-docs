...
- When you choose the main color for a low-code app, ensure there is enough contrast between the header, sidebar, and buttons.
...
- Limit templates to a maximum of 3 visible actions. If more actions are needed, add them behind a context menu.
  ...
  > [!TIP]
  > Tutorial: [Styling a table component - Step 7: Add a context menu](xref:Tutorial_Apps_Style_A_Table#step-7-add-a-context-menu)
...
- Test your layout on different screen resolutions to ensure it displays correctly on all target devices before publish a low-code app.

## Navigation

- Use the sidebar as the main navigation to the different pages defined in the app.

- When there are too many pages in your sidebar or you want to show the same data in another way, use the header bar as tabs to open hidden pages.
