...
- Next to columns containing discrete values of type string or number, you can find a button that allows you to change how the possible values are displayed when you hover your mouse over it:
  ...
  > [!NOTE]
  > From DataMiner 10.5.0 [CU18]/10.6.0 [CU6]/10.6.9 onwards<!--RN 45388 + 45835-->, if the discrete values for a column are marked as strict, the toggle button is not available. The values are shown as a checklist only, and it is not possible to switch to free-form text.

- Next to columns containing Boolean values, you can select the *true* and *false* options as a checklist.

  > [!NOTE]
  > From DataMiner 10.5.0 [CU19]/10.6.0 [CU7]/10.6.10 onwards<!--RN 46155-->, when the [GQI DxM](xref:GQI_DxM) is enabled, selecting both options applies an "is one of" filter with both values, filtering out rows with an empty cell in that column. The filter selection is also stored in the URL and restored when the dashboard is reloaded. In earlier versions, or when the GQI DxM is not enabled, no filter is applied when both options are selected.
