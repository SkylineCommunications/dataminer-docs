...
Proactive cap detection monitors the global trend in the real-time incoming data of a metric and will automatically generate a notification when the metric is predicted to reach a (critical) low or high value when the trend is extended into the future.

To give an accurate estimate of the timing of future issues, it uses a prediction model based on real-time and average trend data stored in the cloud with [STaaS](xref:STaaS) (recommended) or in a self-managed Cassandra-compatible database.

## Prerequisites

For the best results, **both real-time and average trending** must be activated on the parameters for which you want proactive cap detection to be available.

## Proactive cap detection in the Alarm Console

By default, the notifications generated by the proactive cap detection feature are displayed in the **Suggestion events** tab of the Alarm Console, along with the notifications for [behavioral anomaly detection](xref:Behavioral_anomaly_detection) and [pattern matching](xref:Pattern_matching). For details on how to view this tab, refer to [Behavioral anomaly detection in the Alarm Console](xref:Behavioral_anomaly_detection#behavioral-anomaly-detection-in-the-alarm-console).

The value of the suggestion event mentions what kind of issue is expected, e.g., predicted breaches of critical alarm thresholds. The value also mentions "Predicted Critical High" or "Predicted Critical Low". For example, below you can see a suggestion events tab that includes several notifications generated by proactive cap detection:

![Suggestion events tab](~/dataminer/images/Suggestion_events_tab.png)<br>*Suggestion events tab in DataMiner 10.6.1*

From 10.4.12/10.3.0 [CU21]/10.4.0 [CU9] onwards, it is possible to have **full alarms** generated with a user-specified severity when trend prediction suggests that a parameter will enter a specific range. For more detailed information, see [Configuring Augmented Operations alarm settings](xref:Configuring_anomaly_detection_alarms).

Please note:

- From DataMiner Cube 10.3.7/10.4.0 onwards, when a trend is projected to reach the minimum or maximum value within a parameter's range, the value of the suggestion event mentions "Predicted maximum value" or "Predicted minimum value", followed by the actual maximum or minimum value of the parameter. The corresponding unit of measurement, as specified in the protocol, will also be included. <!-- RN 36440 --> Prior to DataMiner Cube 10.3.7/10.4.0, the value of the suggestion event mentions "Predicted above range violation" or "Predicted below range violation".
...
In DataMiner Cube, you can configure this feature in System Center, via *System settings* > *analytics config* > *proactive cap detection*. The following settings are available there:
...
> [!NOTE]
>
> - To have proactive cap detection generate automatic notifications for predicted critical low or critical high values, the [trend prediction](xref:Trend_prediction) feature also needs to be enabled in System Center.
