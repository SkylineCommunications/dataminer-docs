---
uid: Trend_prediction
---

# Trend prediction

With the trend prediction feature, trend graphs can show how the value of a parameter in the graph is most likely to evolve in the future. The predicted trend is generated by a machine learning model trained on the historical trend data of the metric.
...
These different prediction types correspond to the different time resolutions in the trend data logged by DataMiner. By having prediction models on different time resolutions, different trend data characteristics and patterns are captured.
...
- In the DataMiner Cube user settings, the ***Trend prediction range* user setting** determines whether the trend prediction range is by default set to "Auto" or if the range needs to be changed manually. See [Trending settings](xref:User_settings#trending-settings).
...
- The first time the prediction exceeds a particular **alarm threshold**, this is indicated with a **triangle-shaped marker**. If multiple trends are displayed, you can hover over the alarm marker to see a tooltip indicating which trend will trigger the alarm. You can generate automatic alarm console notifications for these predicted alarm threshold breaches. See [Proactive cap detection](xref:Proactive_cap_detection) for more information.
...
## Enabling or disabling trend prediction

To enable or disable the trend prediction feature:
