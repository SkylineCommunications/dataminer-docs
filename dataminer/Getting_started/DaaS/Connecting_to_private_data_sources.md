...
If you are using a DaaS system, by default you will only be able to connect to data sources that can be accessed from dataminer.services over the internet. To make sure DaaS can also access private data sources, you can set up and use **Edge Manager**.

**Prior to DataMiner 10.5.10/10.6.0**, or in some specific cases [not supported by Edge Manager](xref:EdgeManagerOverview#supported-connection-types-and-limitations), a **site-to-site VPN connection** will need to be set up. This will establish a secured connection between your DaaS system and your self-hosted network (which can be on-premises or hosted by the cloud provider of your choice).

## About Edge Manager

Edge Manager offers a simple, fast path to connectivity where flexibility and ease of setup are key. It sets up secure communication tunnels enabling communication with selected on-premises data sources from a DaaS system.

From **DataMiner 10.5.10/10.6.0** onwards, Edge Manager is available on DaaS systems by default. It requires the deployment of a DataMiner Edge Node on premises to establish the secure communication tunnels.
