...
> [!WARNING]
> An *unattend.xml* file stores the administrator password in a recoverable form (Base64-encoded, not encrypted). Anyone with access to the file, or to the virtual hard disk it is injected into, can retrieve that password. Take the following precautions:
>
...
1. Create an answer file named *unattend.xml*, based on the [Windows unattended installation reference](https://learn.microsoft.com/en-us/windows-hardware/manufacture/desktop/windows-setup-automation-overview), that configures at least the following settings for the built-in Administrator account and computer name:
...
1. Attach the modified virtual hard disk to a new VM, as described under [Creating the VM](xref:Using_a_pre_installed_DataMiner_Virtual_Hard_Disk#creating-the-vm), and start it.
   ...
   Windows Setup automatically picks up `Windows\Panther\Unattend\unattend.xml` and uses it to complete the OOBE without any manual interaction.
