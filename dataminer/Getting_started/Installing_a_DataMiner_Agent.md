...
- [Deploy on AWS using the Amazon Machine Image (AMI)](xref:Using_aws_ami)

The table below outlines the main differences between these installation methods.

| [DataMiner Installer](xref:Installing_DM_using_the_DM_installer) | [Pre-installed DataMiner Virtual Hard Disk](xref:Using_a_pre_installed_DataMiner_Virtual_Hard_Disk) | [AWS AMI](xref:Using_aws_ami) |
| -- | -- | -- |
| Allows installation on an existing machine. | Requires a new VM. | Requires a new EC2 instance on AWS. |
| Storage options:<br>- [StaaS](xref:STaaS) (recommended)<br>- Self-managed storage with [dedicated clustered storage setup](xref:Configuring_dedicated_clustered_storage) (requires expert knowledge) | Storage options:<br>- [StaaS](xref:STaaS) (recommended)<br>- Self-managed storage with [dedicated clustered storage setup](xref:Configuring_dedicated_clustered_storage) (requires expert knowledge)<br>- Self-managed storage deployed on the same Windows machine (for small-scale setups only) | Storage options:<br>- [StaaS](xref:STaaS) (recommended)<br>- Self-managed storage with [dedicated clustered storage setup](xref:Configuring_dedicated_clustered_storage) (requires expert knowledge) |
