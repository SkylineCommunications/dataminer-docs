...
For a brand-new Failover pair where the primary DMA is not yet part of a cluster, the correct procedure is to first add the primary DMA to the cluster using its physical IP. Only after that step should you set up the Failover by using the Failover button and adding the backup (empty) DMA.
