...
From DataMiner 10.6.10/10.7.0 onwards<!-- RN 46061 -->, credential secrets are no longer stored in the legacy *Library.xml* file, but as authenticated ciphertext (AES-256-CBC with HMAC-SHA-256) in the system storage itself. In a dedicated clustered storage or STaaS setup, they are kept separately from the data that references them. On systems with storage per DMA, which have no such centralized storage, the secrets are stored in `EncryptedSecret.xml` files in the `C:\Skyline DataMiner\Security\Credentials` folder instead. In all cases, the encryption keys remain on the node itself, in an `encryptors.bin` file in the `C:\Skyline DataMiner\Security\Encryption` folder, wrapped with the Windows [Data Protection API (DPAPI)](https://learn.microsoft.com/en-us/dotnet/standard/security/how-to-use-data-protection) under *LocalMachine* scope, so it cannot be read on another host.
