...
The secrets themselves are kept apart from the records that reference them. Wherever a credential is stored, the record only contains a placeholder identifier (a GUID) that points to the encrypted secret, so no secret value is present in the database tables, in update packages, or in replication events. On systems that store this data in XML files, the encrypted secrets are collected in a single `EncryptedSecret.xml` file in the `C:\Skyline DataMiner\Security\Credentials` folder<!--RN 44349-->.
