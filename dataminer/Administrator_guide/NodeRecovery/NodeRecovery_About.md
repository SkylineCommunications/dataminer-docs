...
DataMiner Node Recovery offers a way to monitor and act upon node outages in the DataMiner System. It will [detect outages](xref:NodeRecovery_Detection), causing the [state of nodes](xref:NodeRecovery_States) to change, which can in turn [trigger scripts](xref:NodeRecovery_Triggers) that deal with the outages.
