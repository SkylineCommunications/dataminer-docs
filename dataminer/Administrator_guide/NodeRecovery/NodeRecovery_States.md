...
In addition to one of these three states, nodes can also be in [maintenance mode](#maintenance-mode). This mode is applied to a node on top of its *Healthy*, *Outage*, or *Unknown* state.
...
| Unknown | Outage | [FirstHeartbeatThresholdMilliseconds](xref:NodeRecovery_Settings#outage-settings) exceeded without receiving a heartbeat. |
| Healthy | Outage | [OutageThresholdMilliseconds](xref:NodeRecovery_Settings#outage-settings) exceeded without receiving a heartbeat. |
