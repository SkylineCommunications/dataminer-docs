...
You can identify the agent that is currently hosting a scheduled task by checking the `ExecutingDmaId` property on the task. This value is displayed in Cube in the Scheduler app in the `DataMiner` column. If you want the task to run on a different agent, you can swarm it to another agent in the cluster.
