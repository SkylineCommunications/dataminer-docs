...
1. From DataMiner 10.6.9 onwards<!--RN 44352-->, if the selected backup contains a passphrase-wrapped encryption envelope (i.e., it was taken on a system with a [DMS Backup Password](xref:Encryption_in_DataMiner#credentials-at-rest) configured) and you are restoring on a different machine, the *DMS Backup Password* field will appear. Enter the DMS Backup Password that was configured when the backup was taken and click *Verify*.

   If the wizard instead displays a warning that encrypted secrets cannot be recovered, the backup does not contain a recoverable encryption envelope. You can still continue, but [credentials encrypted at rest](xref:Encryption_in_DataMiner#credentials-at-rest) will remain unreadable on the restored DMA until a peer DataMiner Agent in the cluster re-synchronizes the encryption material.
