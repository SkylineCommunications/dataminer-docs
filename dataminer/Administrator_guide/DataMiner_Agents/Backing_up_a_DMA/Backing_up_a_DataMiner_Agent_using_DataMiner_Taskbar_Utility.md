...
From DataMiner 10.6.10/10.7.0 onwards<!--RN 46061-->, the *DMS Backup Password* field must be configured in DataMiner Cube under *System Center* > *Backup* before a backup is taken. As a result, you may encounter the following issues:
...
- **Backup taken before DataMiner 10.6.10/10.7.0 or before the password was set**: The archive contains no encryption envelope. The restore will continue and warn that encrypted secrets cannot be recovered. Take a new backup once the DMS backup password is configured.
