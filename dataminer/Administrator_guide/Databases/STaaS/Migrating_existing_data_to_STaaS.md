...
1. When all storage types report **State=Completed**, from DataMiner 10.6.8/10.7.0 onwards<!-- RN 45507 -->, run the *CloudStorageMigrationFinalize* script.
   ...
   This script will finalize the migration by migrating some configuration data, the credentials from the Credential Library (from 10.6.10/10.7.0), doing some checks, etc. After you have run the script, **immediately continue with the next steps** because any configuration changes done after this finalization and before the next restart might otherwise be lost.
