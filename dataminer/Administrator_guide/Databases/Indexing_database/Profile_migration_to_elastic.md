...
If the migration should fail for any reason, the migration status object in the SLNetClientTest tool window will get a red background color. On the agent you started the migration, the ``SLMigrationManager.txt`` and ``SLProfileManager.txt``, but also the ``SLNet.txt`` and ``SLDBConnection.txt`` log files will contain more information.
