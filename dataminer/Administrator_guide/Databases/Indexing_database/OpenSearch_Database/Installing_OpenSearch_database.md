...
> [!TIP]
> These steps do not apply if you just want to renew your OpenSearch certificates. For that, refer to [Renewing the TLS certificates for OpenSearch](#renewing-the-tls-certificates-for-opensearch)
...
To renew your self-signed TLS certificates for an existing OpenSearch database, you will have to replace the existing certificates with newly signed ones. The expired certificates can no longer be used. To generate new certificates, we recommend using the *Generate-TLS-Certificates* script.

1. Generate the certificates using the [Generate-TLS-Certificates](https://github.com/SkylineCommunications/generate-tls-certificates) script on GitHub.

   This script will generate a .p12 file for every node and a .pem admin cert and key file that is used for security configuration settings. The GitHub script will provide you with the keystore and truststore password.

   A rootCA.crt can also be generated by the GitHub script; however, we recommend using the original *rootCA.crt* (the same one used on the previous renewal/certification). If this is not possible, the new *rootCA.crt* will have to be applied on all DataMiner and OpenSearch nodes.

1. Remove the outdated certificates from the OpenSearch nodes.

   These are usually located in the `\etc\opensearch\cert` folder, but not always. You can find the exact location of the certificates in the TLS segment of the *opensearch.yml* file.
...
1. Navigate to the location where the generated *rootCA.crt* file is stored, and copy it.

   On Ubuntu, you can do so using the following command: `sudo cp rootCA.crt /usr/local/share/ca-certificates/`

1. On every Linux OpenSearch node, update the trusted root certificates.

   On Ubuntu, you can do so using the following command: `sudo update-ca-certificates`
