...
| SourceId | smallint(6) | No | | A value from the slenumvalues table indicating the module that generated the alarm:<br> -  16 (DataMiner System)<br> -  23 (Correlation Engine)<br> -  30 (External)<br> -  ...<br> See [slenumvalues table](#slenumvalues-table). |
...
| ROOTKey | bigint(20) | No | | The ID of the original alarm in the alarm’s history tree. |
| chIndex | text | Yes | | If the alarm is associated with a dynamic table parameter, this field will contain the display key of the table row.<br> IMPORTANT: If it concerns a dynamic table parameter with “advanced naming” enabled, then this field will contain the primary key instead of the display key. |
| rcaLevel | int(11) | Yes | | The proximity of the alarm to the root cause of the problem, i.e., the position of the alarm in the Root Cause Analysis connectivity chain. |
...
> [!NOTE]
> This table is not currently supported in Swarming-enabled environments. See [Offload database configuration with Swarming enabled](xref:Offload_Database_With_Swarming) for more information.
...
| Toa | datetime | No | | Date/time at which the information event appeared in the DataMiner System. |
...
| UserStatus | smallint(6) | Yes |  | A value from the slenumvalues table indicating the current ownership status:<br> -  18 (Not assigned)<br> -  19 (Acknowledged)<br> -  21 (Unresolved)<br> -  ...<br> See [slenumvalues table](#slenumvalues-table). |
| ROOTKey | bigint(20) | No | | Not applicable in case of information events. |
| chIndex | text | Yes | | If the information event is associated with a dynamic table parameter, this field will contain the display key of the table row.<br> IMPORTANT: If it concerns a dynamic table parameter with “advanced naming” enabled, then this field will contain the primary key instead of the display key. |
| rcaLevel | int(11) | Yes | | Not applicable in case of information events. |
