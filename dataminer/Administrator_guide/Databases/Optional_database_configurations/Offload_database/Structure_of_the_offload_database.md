...
> [!NOTE]
> Before 10.6.4, the `EId` column was not part of the primary key. Tables created before this version will keep working, but will require an update when enabling the Swarming feature. See [Offload database configuration with Swarming enabled](xref:Offload_Database_With_Swarming) for more information. 
