...
| MaxAmountOfInstancesReached | The function definition of the resource has reached its maximum instances. If the function definition has an entrypoint table configured, the maximum number of instances will be equal to the number of rows in the entrypoint table. If no entrypoint table is configured, it will be defined by the *maxInstances* property in the function definition. |
