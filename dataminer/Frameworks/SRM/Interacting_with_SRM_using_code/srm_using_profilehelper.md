...
|---|---|---|
| ProfileParameterUsedAsCapabilityInResource | Occurs when attempting to delete or update a profile parameter that is used in a resource currently involved in a scheduled or ongoing reservation. The delete or update action is blocked. | *ProfileParameterID* - The ID of the parameter that could not be deleted or updated. *ResourceIds* - The IDs of the resources using the parameter, preventing the profile parameter from being edited. |
| ProfileParameterUsedAsCapacityInResource | Occurs when attempting to delete or update a profile parameter that is used in a resource currently involved in a scheduled or ongoing reservation. The delete or update action is blocked. | *ProfileParameterID* - The ID of the parameter that could not be deleted or updated. *ResourceIds* - The IDs of the resources using the parameter, preventing the profile parameter from being edited. |
| ProfileUpdateFailed | Occurs when attempting to update a profile instance that is currently involved in a scheduled or ongoing reservation. | *ResourceManagerErrorData* - Contains the trace data generated by the ResourceManager when the update was applied. |
| ReservationsMustMovedToQuarantine |  Occurs when attempting to update a profile instance that is currently involved in a scheduled or ongoing reservation and the update cause the reservation to go to quarantine | *ResourceManagerErrorData* - Contains the reservations that need to be moved to quarantine. |
| ModuleNotInitialized | ProfileManager is not initialized yet. | None |
| InvalidServiceProfileDefinitionLink | The linked service profile definition does not exist or is not valid because of existing service profile instances. | *ProfileDefinition* -The profile definition you attempted to delete or update. *ServiceProfileDefinitionID* - The ID of the service profile definition that did not exist or was invalid. *ProfileInstanceIDs* - The IDs of profile instances that reference a service profile instance no longer compatible with the new service profile definition. |
| InvalidServiceProfileInstanceLink | The linked service profile instance does not exist or is not valid because of the service profile definition of the profile definition. | *ProfileInstance* - The profile instance you attempted to delete or update. *ServiceProfileInstanceID* - The ID of the service profile instance that did not exist or was invalid. *ServiceProfileDefinitionID* - The ID of the service profile definition configured on the profile definition of the profile instance, which caused the service profile instance to be invalid. |
...
| InvalidParameterOnTableDefinition | One of the parameters specified on the table definition does not belong to the profile definition. | *ProfileDefinition* - The profile definition you attempted to create or update. *ProfileParameterID* - The ID of the profile parameter not found in the profile definition. |
...
| InvalidValueReference | The profile instance references a value that could not be resolved. | *ProfileInstance* - The profile instance with the invalid value reference. *ProfileParameterID* - The ID of the profile parameter containing the invalid value reference. |
| RemovedProfileParameterInUse | The profile definition removes a profile parameter while at least one profile instance exists that is dependent on that profile definition parameter. | *ProfileDefinition* - The profile definition requesting parameter removal. *ProfileInstanceIDs* - The IDs of profile instances affected by the removal of parameters from their associated profile definition. |
