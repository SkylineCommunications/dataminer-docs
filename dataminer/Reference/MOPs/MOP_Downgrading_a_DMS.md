...
> - If you are downgrading because an upgrade introduced issues affecting the Credentials Library, restore the `Library.xml` file from the [recycle bin](xref:Recycle_bin) to the `C:\Skyline DataMiner\Security\Credentials` folder on all DataMiner Agents in the cluster. This restores the Credentials Library to its pre-upgrade state. Any Credentials Library changes made after the upgrade will be lost.
