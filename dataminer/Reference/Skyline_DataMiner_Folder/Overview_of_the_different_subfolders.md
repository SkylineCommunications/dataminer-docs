...
| Security | Within this folder, only the *Credentials* subfolder, which contains the credentials library configuration, and the *Ownership* subfolder are synchronized. From DataMiner 10.6.9 onwards<!--RN 44075-->, the *Encryption* subfolder additionally contains a backup copy of the DPAPI-protected encryption material that DataMiner uses to [encrypt credentials at rest](xref:Encryption_in_DataMiner#credentials-at-rest). | Partially |
