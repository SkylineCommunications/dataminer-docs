...
| Security | Within this folder, only specific subfolders are synchronized:<br>- The *Credentials* subfolder, which contains the Credentials Library configuration.<br>- The *Ownership* subfolder.<br>- From DataMiner 10.6.10/10.7.0 onwards<!--RN 46061-->, the *Encryption* subfolder, which contains a backup copy of the DPAPI-protected encryption material that DataMiner uses to [encrypt credentials at rest](xref:Encryption_in_DataMiner#credentials-at-rest). | Partially |
