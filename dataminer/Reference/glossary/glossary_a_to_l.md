...
| Catalog | Central repository for components that can facilitate and accelerate the use of DataMiner, including the complete connector database. See [Catalog](xref:About_the_Catalog_app). |
...
| Concurrency | Used in the context of DataMiner SRM to indicate the number of times a resource can be used at the same time. See [Service and Resource Management](xref:About_SRM). |
...
| Connector | An XML file that allows a DMA to communicate with a data source in the system. Also known as protocol or driver. See [Protocols](xref:Protocols1). |
...
| Correlated alarm | Alarm generated by the DataMiner Correlation engine, based on a correlation rule triggered by one or more other alarms. See [Correlated alarms](xref:Alarm_types#correlated-alarms). |
| Correlation | DataMiner module that can gather information about parameter values and alarms, and trigger specific actions, such as generating a new, correlated alarm, based on triggers defined by the user. See [Correlation](xref:About_DMS_Correlation). |
...
| Correlation rule | Rule defining when DataMiner Correlation should be triggered and what should happen when it is triggered. See [About DataMiner Correlation](xref:About_DMS_Correlation). |
...
| DaaS | [DataMiner as a Service](xref:DaaS_hosting). A cloud-native, fully operational DataMiner System that you can deploy with a single click. |
| DAB | Digital Audio Broadcasting. Type of audio broadcasting where analog audio is converted to a digital signal. |
| Dashboard | A custom page displaying various information about the monitored system. See [Dashboards app](xref:newR_D). |
...
| DataMiner Live Data Sharing Service | Former name of the Sharing app.  See [Sharing](xref:About_the_Sharing_app). |
...
| DCF | DataMiner Connectivity Framework. Framework used to manage element connectivity, e.g. to configure and visualize signal paths within an infrastructure. See [DataMiner Connectivity Framework](xref:About_the_DataMiner_Connectivity_Framework). |
...
| DIS | DataMiner Integration Studio. Microsoft Visual Studio extension that turns Visual Studio into a DataMiner protocol IDE. See [DataMiner Integration Studio](xref:Overall_concept_of_the_DataMiner_Integration_Studio) |
...
| DVE | [Dynamic Virtual Element](xref:Dynamic_virtual_elements). An element from which child elements are dynamically generated. The term DVE is used both for the parent and the child elements. |
| DxM | [DataMiner Extension Module](xref:DataMinerExtensionModules). A DataMiner component that can be installed next to an existing DataMiner installation to enable additional functionality. A DxM can be installed and upgraded independently from the DataMiner version and without any downtime of the core DataMiner software. |
| Dynamic alarm threshold | Alarm threshold that is not based on a fixed value, but defined in comparison with a baseline value. See [Configuring dynamic alarm thresholds](xref:Configuring_dynamic_alarm_thresholds). |
...
| Engineered path routing | Network routing in the context of [DataMiner SRM](xref:About_SRM) where the path between the fixed source and destination is determined via automated path routing.. |
...
| EPM | [Experience and Performance Management](xref:EPM). Software platform specifically designed to manage very high volumes of devices in cable, telco, terrestrial and satellite environments. Previously known as CPE in legacy DataMiner versions. |
...
| Fixed path routing | Network routing in the context of [DataMiner SRM](xref:About_SRM) where the functions required in the path are statically defined in the service definition. |
...
| Function DVE | Alternative name for "virtual function". See [Service and Resource Management](xref:About_SRM). |
| Function hysteresis | Functionality of [DataMiner SRM](xref:About_SRM) that keeps virtual function resources from being disabled if they will be used again soon. See [Advanced SRM settings](xref:Function_resource_settings). |
| Function XML | XML file containing a virtual function definition. See [Service and Resource Management](xref:About_SRM). |
...
| GQI | [Generic Query Interface](xref:About_GQI). A special type of data for DataMiner Dashboards and Low-Code Apps. |
| Grandmaster clock | Device from which the time is retrieved in a PTP environment. See [DataMiner PTP app](xref:SolPTP). |
...
| Job Manager | Legacy DataMiner application used to create and manage jobs. Commonly referred to as the "Jobs app". See [Job Manager](xref:jobs). |
| Job template | A job configuration, used Job Manager, that is already partly filled in, so that only a few fields still need to be completed to have a fully configured job. See [Working with job templates](xref:Working_with_job_templates). |
