...
> [!IMPORTANT]
> This tool is deprecated. If your system still uses Elasticsearch, use the [Elasticsearch snapshot functionality](xref:Configuring_Elasticsearch_backups_Windows_Linux) instead. Keep in mind that support for Elasticsearch ends in DataMiner 10.7.x. We recommend using [Storage as a Service](xref:STaaS) instead, or, if your setup requires self-managed storage, using [OpenSearch](xref:OpenSearch_database).
