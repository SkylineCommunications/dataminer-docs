...
1. If the change follows a product schema, API, package, validator, template, or behavior change, create or review the D6.2 product-change input. Record the affected documentation UIDs and areas, source release and package identity, acknowledgement, and documentation update decision. Use `unknown` with a follow-up when a cross-repository value is not confirmed.

   Run `.\scripts\validate-documentation-dependency-map.ps1 -RepositoryRoot . -ChangePath .\product-change.json` and resolve the report with `.\scripts\resolve-documentation-coupling.ps1 -RepositoryRoot . -ChangePath .\product-change.json -FailOnUnmapped -FailOnGate`. Run the targeted checks listed in `targetedChecks`.
...
1. Specify a title for the pull request, and optionally add a comment with more information about your changes.
   ...
   Include the affected workstream, generated or source-contract impact, D6.2 coupling report and gate status, validation results, and any owner, authority, stale-content, or unresolved-conflict follow-up in the description.
