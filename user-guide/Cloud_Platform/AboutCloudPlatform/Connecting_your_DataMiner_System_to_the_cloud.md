...
1. Verify that the following **requirements** are met:
   ...
   - [DataMiner CloudGateway](xref:DataMinerExtensionModules#cloudgateway) V2.9.6 onwards requires the internal network to allow [HTTP(S) traffic via port tcp/5100](xref:Configuring_the_IP_network_ports#overview-of-ip-ports-used-in-a-dms) used for a tunneling proxy towards the cloud. More information can be found on [DataMiner CloudGateway](xref:DataMinerExtensionModules#cloudgateway).
