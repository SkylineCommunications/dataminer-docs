...
1. Verify that the following **requirements** are met:
   ...
   - [DataMiner CloudGateway](xref:DataMinerExtensionModules#cloudgateway) 2.10.0 and onwards requires the internal network to allow [HTTP(S) traffic via port tcp/5100](xref:Configuring_the_IP_network_ports#overview-of-ip-ports-used-in-a-dms) which is used as a cloud endpoint for other [DataMiner Extension Modules](xref:DataMinerExtensionModules#dataminer-extension-modules-dxms). For more information about configuring this endpoint, please refer to [Custom cloud endpoint configuration](xref:Custom_cloud_endpoint_configuration).
