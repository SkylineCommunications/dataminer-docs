...
Users with a DataMiner Cloud Platform account (i.e. users who are registered on <https://dataminer.services>) can be given access to cloud features such as remote access to the DataMiner Monitoring app, using the [Teams bot](xref:DataMiner_Teams_bot) to interact with a DataMiner System, etc. However, before they can access these features, you must add their DataMiner Cloud Platform account to your organization and to the DataMiner System. In addition, remote access must be enabled for the DataMiner System.
...
1. If remote access has not been enabled yet, make sure to do so first. See [Controlling remote access with the Admin app](xref:Controlling_remote_access).
...
> [!NOTE]
> If users have been given cloud access to a DMS, to access the Monitoring app, they need to go to the public address of the cloud-connected DMS, in the format `https://[dms_name]-[company_name].on.dataminer.services`. This address can be found on the *Overview* page for the DataMiner System in the Cloud Admin app.
>
