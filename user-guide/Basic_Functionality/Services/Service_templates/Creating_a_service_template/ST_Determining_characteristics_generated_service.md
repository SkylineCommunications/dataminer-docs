...
- To determine which Visio file will be used on the Visual Overview pages of the service card, specify the file under *Visio File*. If necessary, click the downward triangle to the right of the box to enter placeholders.
  ...
  > [!NOTE]
  > To upload a ZIP or VDX file, fill in the file name without the file extension. To use a VSDX file, specify the file name with the .vsdx extension. In addition, to upload a VDX file, make sure it is located in the folder `C:\Skyline DataMiner\Views` of the DataMiner Agent.
