...
From DataMiner 10.3.7/10.4.0 onwards, the value of the suggestion event mentions the predicted maximum or minimum value of the parameter, along with the corresponding unit of measurement, as specified in the protocol. Prior to DataMiner 10.3.7/10.4.0, the value of the suggestion event mentions what kind of issue is expected, e.g. a critical high or low alarm or an above or below range violation.

The value also mentions between which times the issue is expected to occur. The closer to the predicted time, the more accurate this prediction will be, so the suggestion event will be automatically updated with more accurate information when appropriate. As soon as the predicted time of the incident has passed, the suggestion event will be cleared.
