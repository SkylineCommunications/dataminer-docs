...
> - For an example of use, refer to the view "Linking Shapes" on the [Ziine Demo System](xref:ZiineDemoSystem). The example can be found on the Visual page _controls > PIE/BAR CHART_.
...
2. Set the value of the **Parameters** shape data field to the parameters you wish to display, using the following syntax:
   ...
   > [!NOTE]
   >
   > - In the value of the shape data field, you can use placeholders to refer to e.g. session variables. For example: _219:341:114:\[Var:MyVariable\]\|219:341:114:\[Var:MyOtherVariable\]_.
...
5. If the chart consists of a group of shapes, add a shape data field of type **ParametersOptions** to each of the individual shapes, and set its value to the appropriate chart component each time:
   ...
   > [!NOTE]
   >
   > - The title in the title component will only be automatically displayed if the option _Title:\<your title>_ is specified in the **ParametersOptions** shape data field of the group, and an asterisk has been added in the title shape. Alternatively, you can also manually specify the title within the title shape.
...
- **Range**:
  ...
  If you want to override those default settings, use the “_Range:_” option to set the range to one of the following options:
...
- **ThemeForeground**:
  ...
  > [!NOTE] > [For more information on the possible color names, refer to the following website:](<https://msdn.microsoft.com/en-us/library/system.windows.media.brushes_properties(v=vs.110).aspx>) https://msdn.microsoft.com/en-us/library/system.windows.media.brushes_properties(v=vs.110).aspx.
...
- **VisualizeExceptions**
  ...
  With this option, you can determine whether the display value of exception values in real-time charts is shown in a label. Can be set to _VisualizeExceptions=true_ (default behavior) or _VisualizeExceptions=false_. Available from DataMiner 10.2.0/10.1.6 onwards;
...
- All basic font properties are taken from the shape, including the color. However, the option “_ThemeForeground_” overrides the text color. See [ThemeForeground:](xref:Creating_a_parameter_chart).
