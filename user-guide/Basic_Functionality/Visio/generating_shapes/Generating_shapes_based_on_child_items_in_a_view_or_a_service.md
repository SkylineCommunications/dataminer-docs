...
> [!NOTE]
>
> - For an example of use, refer to the view "Linking Shapes" on the [Ziine Demo System](xref:ZiineDemoSystem). The example can be found on the Visual page _children > ELEMENTS AND VIEWS_.
...
- **ChildType**: In this mandatory shape data field, specify the type of child item the shape has to represent: view, element, service or redundancy group.
  ...
  Example: All child items of type “Element” will be represented by the shape of which **ChildType** is set to “_Element_”.
...
- **ChildrenFilter**:
  ...
  <br>For example, _Name=\[var:userSpecifiedName\]_. Only objects of which the name matches the regular expression will be shown.
...
- **Children**: In this mandatory shape data field, specify the type of child items that have to be generated: _View_, _Element_, _Service_ and/or _RedundancyGroup_. In case of multiple types, separate them by pipes.
  ...
  > - With the DataMiner Cube user setting _Maximum number of child shapes in a 'Children' container shape_, you can control the maximum number of Visio shapes allowed in a **Children** container shape. Default: 100. See [Visual Overview settings](xref:User_settings#visual-overview-settings).
...
- **ChildrenSort**: In this optional shape data field, you can specify how the different child item shapes should be sorted:
  ...
  - If you add a shape data field of type _ChildrenSort_, and configure the following value, the shapes will be sorted by element name in descending order (i.e. Z to A):
