...
> [!NOTE]
> From DataMiner 10.3.6/10.4.0 onwards, the light bulb icon is displayed in the top-right corner of the trend graph regardless of whether certain mandatory prerequisites are still missing. <!-- RN 35868 --> However, clicking the icon will prompt a message indicating that the necessary requirements have not been met, preventing you from using this feature without them. <!--RN 36157-->Prior to DataMiner 10.3.6/10.4.0, the light bulb icon will not appear until all mandatory requirements are met.
...
> [!NOTE]
>
> - From DataMiner 10.3.6/10.4.0 onwards, you can check if any of these (optional) requirements are still missing in your system by clicking the light bulb icon in the top-right corner of the trend graph. If any of these requirements are missing, a message will appear advising you to unlock all capabilities of this feature. Note that this message will not appear if the mandatory prerequisites are also still missing. <!-- RN 35868 --> Prior to DataMiner 10.3.6/10.4.0, you can check if any of these (optional) requirements are still missing in your system by checking the logs via *System Center* > *Logging* > *cube*. Filter the logs on "requirements" to quickly find the relevant entries.
