...
> - You can enable or disable this feature via *System Center* > *System settings* > *analytics config.* However note that the Trend Prediction feature needs to be enabled in order to receive automatic notifications for predicted alarm threshold breaches by [Proactive cap detection](xref:Proactive_cap_detection).
...
> - A high-precision prediction (prior to DataMiner 10.0.12: short-term prediction) is generated by a model fit on the real-time trend data. This prediction has the most details but also the shortest forecast time horizon.
> - A short-time prediction (prior to DataMiner 10.0.12: mid-term prediction) is generated by a model fit on the 5-minute averaged trend data. Since 5-minute averages are usually logged across longer time spans, the forecast time horizon in a short-time prediction is typically longer than for a high-precision prediction.
> - A mid-term prediction (prior to DataMiner 10.0.12: long-term prediction) is generated by a model fit on the 1-hour averaged trend data. The forecast time horizon in a mid-time prediction is typically longer than in a short-term prediction.
> - A long-term prediction is generated by a model fit on an aggregated set of 1-hour trend data points and extrapolates the global trend in the data to the future.
...
From DataMiner 9.5.13 onwards, the default trend prediction time range setting can be configured in the user settings. See [Trending settings](xref:User_settings#trending-settings).

> [!NOTE]
>
> - One or multiple of the prediction types might be unavailable for your metric. This is the case when there is an insufficient number of trend data points available in order to build a reliable model or when the data behavior cannot be captured in an accurate way by a prediction model.
...
> - Automatic alarm console notifications for these predicted alarm threshold breaches can be generated: see [Proactive cap detection](xref:Proactive_cap_detection) 
