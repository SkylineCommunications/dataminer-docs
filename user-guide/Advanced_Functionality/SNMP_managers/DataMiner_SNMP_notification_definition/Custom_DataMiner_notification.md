...
| Table display index| 1.3.6.1.4.1.8813.1.1.2.4.1.13| Depending on how table is indexed, would return the following: <br> - Primary key only: returns primary key value.<br> - Table with naming option, but no display key column: returns what has been defined in naming option.<br> - Table with naming option and column of type DisplayKey: It will return the value in the display key column.|
