...
This tutorial shows how you can get started with DataMiner Connectivity Framework (DCF) based on a basic example. In this example, using elements that expose external DCF interfaces, we will create DCF connections between these elements and visualize them using an overview (Visio) in Cube and a node edge graph component in a dashboard/LCA.
...
>[!TIP]
>See also: Kata #43: DataMiner Connectivity Framework (DCF) on DataMiner Dojo.

## Prerequisites

- To be able to expose DCF interfaces and connections in dashboards/LCA, you may need to activate the [GenericInterface](xref:Overview_of_Soft_Launch_Options#genericinterface) soft-launch option. See [Soft-launch options](xref:SoftLaunchOptions).

>[!TIP]
>If you sue a DaaS system, these prerequisites are automatically met.
...
The objective of this tutorial is to build the following network connectivity diagram in a visual overview (Visio) and a node edge graph using DCF:
...
For example, from the diagram we can infer the following:

- RO-01 connects to SW-01 using **Port Ethernet 1/1**

- SW-01 connects to RO-01 using **Port Ethernet 1/1**

- SW-01 connects to PC-11 using **Port Ethernet 1/2**

- PC-11 connects to SW-01 using **Port Ethernet 1/1**
...
- Connectors: Generic Data Simulator. This connector exposes DCF interfaces and will be used by the elements created for this tutorial.

- Automation script: **SLC-AS-Kata-DCF**. This script will be used to provision the view and elements required for this tutorial.

- Visio file: **kata_dcf_view.vsdx**. This Visio file will be used to visualize DCF connections in Cube.

- Dashboards: These will be located in the folder *KATA_DCF*:

  - **KATA_DCF**: A blank dashboard containing all GQI queries required to create the node-edge diagram for this tutorial.

This dashboard contains the following GQI queries:
...
1. Provision the components required for this tutorial.

In the **Automation** module, go to the folder **Kata DCF** and execute the script **SLC-AS-Kata-DCF**.

>[!Tip]
>More information about how to execute automation scripts can be found in DataMiner Docs

After executing the automation script, the following items should be available in the surveyor:
...
Below a screenshot from the surveyor showing these components:
...
>[!Tip]
>
>More information about how to set as active a Visio file can be found in [Editing a Visual Overview in DataMiner Cube](xref:Editing_a_visual_overview_in_DataMiner_Cube).
...
## Create External DCF Connections
...
>[!Note]
>You will notice that there is already a connection in interface **Port Ethernet 1/1**. This is because DCF creates both connection on both ends.
...
## Create Node Edge graph using DCF connections
...
## Display KPIs related to connection
