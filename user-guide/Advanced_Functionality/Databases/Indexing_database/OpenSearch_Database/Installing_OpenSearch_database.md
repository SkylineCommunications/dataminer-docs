...
> [!IMPORTANT]
>
> - For new installations of OpenSearch 2.12 and later, you must define a custom admin password in order to set up a demo security configuration. This is documented in the step to [Install OpenSearch from a package step 2](https://opensearch.org/docs/latest/install-and-configure/install-opensearch/debian/#install-opensearch-from-a-package).
...
> [!NOTE]
> It's also possible to use pem certificates to secure your OpenSearch. You can find more details on how to do this in the [X.509 Pem Certificates section](https://opensearch.org/docs/latest/security/configuration/tls/#x509-pem-certificates-and-pkcs-8-keys).
...
1. Generate the certificates using the [Generate-TLS-Certificates](https://github.com/SkylineCommunications/generate-tls-certificates) script on GitHub.
   ...
   This script will generate a .p12 file for every node and an admin file. The GitHub script will provide you with the keystore and truststore password. A rootCA.crt will also be generated by the GitHub script or you can select your own RootCA.crt during the configuration.
...
> [!NOTE]
> Below commands are specific for Ubuntu. The folder to store the certificate can change between different Linux versions.
...
##### Restart OpenSearch & Apply config
...
1. When you have completed the steps above, restart OpenSearch and apply the user settings using the following commands, making sure to replace the placeholders `<IPOfYourNode1>`, `<FQDNOfYourNode-node-keystore.p12>`, and `<GeneratedPasswordByGithubScript>` with the appropriate data:
  ...
  > [!NOTE]
  > Full details, or explanation on how to do this with pem files can be found on the [security admin page](https://opensearch.org/docs/latest/security/configuration/security-admin/).
