...
> [!NOTE]
>
> - If you use a MSSQL database, a Microsoft SQL Server Enterprise Edition license is required if partitioned data tables are used. Otherwise, the Community edition is sufficient. To run Microsoft SQL Server 2019, Windows Server 2016 or higher is required.
> - If you use an MSSQL database, and instead of user accounts of type NT AUTHORITY\\SYSTEM you want to use other domain accounts, either use a connection string containing "*Integrated Security=true;user id:Domain\\User Name;Password:PWD*", or set the tag *\<IntegratedSecurity>* to true in *DB.xml*.
