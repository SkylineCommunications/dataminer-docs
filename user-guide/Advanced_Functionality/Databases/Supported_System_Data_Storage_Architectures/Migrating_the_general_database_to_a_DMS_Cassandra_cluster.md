...
1. Take a snapshot of the entire Elastic cluster as explained in [Taking a snapshot](xref:Configuring_Elasticsearch_backups_Windows_Linux#taking-a-snapshot).

1. Restore the snapshot on the target remote Elastic. For more information see [Restoring a snapshot](xref:Configuring_Elasticsearch_backups_Windows_Linux#restoring-a-snapshot).

1. Verify whether the snapshot restore worked by [comparing the indices for the local Elastic with those of the target Elastic](http://[IPaddress]:9200/_cat/indices).

   These indices should be identical and take up the same amount of space.

1. At this point, take note of the prefixes used in the indices. In a further step, we will need this information. The default prefix is "dms".
...
1. In the pop-up window, enter the Cassandra and Elasticsearch settings, and click *Confirm*. The following settings are required:
   ...
   - Elasticsearch settings: These are not used in case there already is an Elasticsearch cluster connected to your DMS. In that case, you can just fill in dummy data.
     ...
     - *Database prefix*: The prefix for the Elasticsearch indices and aliases. Enter: `elastic_prefix_here`.
       ...
       > [!NOTE]
       > By entering `elastic_prefix_here`, DataMiner will use the same prefix as mentioned in step 4 to identify which data it owns. If a different prefix is entered, it will be impossible to use the migrated data.

     - *Elastic user*: The Elasticsearch username.

     - *Elastic password*: The password for the specified username. This is not mandatory, unless manually specified.

       ![elastic_prefix_here](~/user-guide/images/elastic_prefix_here.png)
