...
Kibana is a browser-based tool that allows you to connect to and query an Elasticsearch database.

In case you suspect there are problems with an Elasticsearch database, it can be useful to connect to the database and query it. If your DMS uses Elasticsearch, this can for instance be useful when the SLDataGateway process shows high CPU usage, when there is a lot of Elasticsearch communication in the network (visible in WireShark), when Elasticsearch uses too many system resources, or when the SRM features in DataMiner seem to respond very slowly.
