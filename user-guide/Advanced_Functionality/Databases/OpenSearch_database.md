...
A rootCA.crt will also be generated by the GitHub script. The rootCA.crt can also be installed on a Windows machine by double-clicking it in case you want to query the OpenSearch from a different computer. As *Store Location* when adding the certificate, use *Local Machine* and place the certificate in *Trusted Root Certification Authorities*.
