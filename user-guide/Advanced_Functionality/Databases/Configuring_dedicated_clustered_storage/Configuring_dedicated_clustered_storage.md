...
> [!NOTE]
> If you deployed DataMiner by using the [pre-installed DataMiner Virtual Hard Disk](xref:Using_a_pre_installed_DataMiner_VHDX) and you chose the [WSL database](xref:Local_database_on_WSL) configuration, both [Cassandra Cluster and OpenSearch run locally on the Virtual Machine on Windows Subsystem for Linux (WSL)](xref:Local_database_on_WSL). Consider [decommissioning WSL](xref:Decommissioning_WSL) so it no longer consumes any resources.
