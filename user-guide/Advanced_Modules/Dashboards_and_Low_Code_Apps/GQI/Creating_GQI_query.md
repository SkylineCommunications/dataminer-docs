...
- From DataMiner 10.3.0 [CU9]/10.3.12 onwards<!--RN 37505-->, you can specify a `LanguageTag` in the `DMAGenericInterfaceQueryFetchOptions` to determine the culture used when executing a GQI query. This `LanguageTag` should contain a [BCP 47](https://www.rfc-editor.org/info/bcp47) string representation of a specific language (e.g. "en", "en-US", etc.), which will then be converted to a C# culture before being passed to GQI. For more information, refer to the [Microsoft .NET documentation](https://learn.microsoft.com/en-us/dotnet/api/system.globalization.cultureinfo.getcultureinfo). If no valid `LanguageTag` value is provided, the invariant culture will be used. By default, the web apps use the browser locale to determine the culture. For more information see [Navigator: language property](https://developer.mozilla.org/en-US/docs/Web/API/Navigator/language). Prior to DataMiner 10.3.0 [CU9]/10.3.12 onwards, the invariant culture is used when executing a GQI query.
