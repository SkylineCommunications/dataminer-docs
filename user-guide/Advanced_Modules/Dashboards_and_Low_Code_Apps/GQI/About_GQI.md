...
The GQI engine can run:

* As an extension module: see [GQI DxM](xref:GQI_DxM).
* Within the SLHelper process: it is only activated when it is used. Once the process has been activated, it remains operational for a duration of 30 minutes before automatically shutting down and releasing any utilized resources.
