...
1. If one or more actions should be triggered when the page is loaded or closed, in the page configuration pane, open the *Events* section and click the *Configure actions* button next to *On open* (or *On page load*) or *On close* (available from DataMiner 10.3.0 [CU14]/10.4.0 [CU4]/10.4.7 onwards<!--RN 39604 + 39682-->). Then configure the action(s) that should be triggered. See [Configuring low-code app events](xref:LowCodeApps_event_config).
