...
1. Apply the necessary data feeds. See [Applying a data feed](xref:Apply_Data_Feed).
   ...
   > [!NOTE]
   > From DataMiner 10.3.0 [CU13]/10.4.0 [CU1]/10.4.4 onwards<!--RN 38811-->, when you apply a single query data feed, the resulting rows from that query are listed instead of the query itself. Additionally, if you want to use this tree feed component as a data feed, the entry will be listed as *Tables* or *Query rows* in the *Data* pane, depending on your DataMiner version<!--RN 41075-->. When you apply multiple query data feeds, the queries themselves are listed as data. Prior to DataMiner 10.3.0 [CU13]/10.4.0 [CU1]/10.4.4, when you apply a single query data feed, the query itself is listed as data. If you use this tree feed component as a data feed, the entry will be listed as *Queries* in the *Data* pane.
