...
Available from DataMiner 10.4.7 onwards<!--  RN 35902 -->. This basic control allows the user to enter text, which will then be available as a string feed in the dashboard or low-code app. The string feed can be used by queries and by script parameters in low-code app actions. This string feed can also be cleared with an 'x' button available in the input.
