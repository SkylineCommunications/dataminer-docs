---
uid: General
---

# General

This category contains the following visualizations:

- [Form](xref:DashboardForm)

- [Block](xref:DashboardBlock)

- [Button](xref:DashboardButton)

- [Button panel](xref:DashboardButtonPanel)

- [Clock (analog)](xref:DashboardClockAnalog)

- [Clock (digital)](xref:DashboardClockDigital)

- [Generic map](xref:DashboardGenericMap)

- [Group](xref:DashboardGroup)

- [Icon](xref:DashboardIcon)

- [Image](xref:DashboardImage)

- [Stepper](xref:DashboardStepper)

- [Text](xref:DashboardText)

- [Web](xref:DashboardWeb)
