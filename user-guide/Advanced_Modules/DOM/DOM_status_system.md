...
> - When the 'ReadOnly' or 'ClientReadOnly' option is enabled, the field will still be assignable in a Low-Code App's create form (but not in the edit form). This behavior is consistent with the non-status system, where it would otherwise be impossible to assign a value. If a field should be hidden during the create form, you must set it as hidden in the initial status and make it visible in subsequent states.
