...
| ValueForSoftDeletedFieldNotAllowed | Attempted to create or update a `DomInstance` with one or multiple `FieldValue`s that are part of a soft-deleted `FieldDescriptor`, `SectionDefinitionLink` or `StatusSectionDefinitionLink`. Available properties: *DommInstanceId*, *AssociatedFields* |
