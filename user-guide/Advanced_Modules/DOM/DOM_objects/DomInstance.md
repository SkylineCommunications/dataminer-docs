...
| ValueTooLarge | Attempted to create or update a `DomInstance` with a string value that is too large. The *AssociatedFields* collection contains the IDs of the `SectionDefinition`, `Section` and `FieldDescriptor` that point to the incorrect `FieldValue`, in addition to the *ActualSize* property that contains the actual UTF-8 size of the received string. |
