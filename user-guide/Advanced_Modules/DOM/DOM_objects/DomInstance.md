...
| FieldValueUsedInDomInstanceLinksToNonExistingFieldDescriptor | Attempted to create or update a `DomInstance` with one or multiple field values that reference a `FieldDescriptor` which does not exist. The *DomInstanceId* contains the ID of the DOM Instance and the *AssociatedFields* collection contains the IDs of the `SectionDefinition`, `Section`, and `FieldDescriptor` that point to the incorrect `FieldValue`. This error message is returned from DataMiner 10.5.6/10.6.0 onwards.<!-- RN 42358 --> |
