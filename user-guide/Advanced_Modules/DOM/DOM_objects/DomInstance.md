...
| DomInstanceDoesNotContainAllRequiredFieldsForSectionDefinition | The given `DomInstance` had a `Section` that did not contain a `FieldValue` for every non-optional `FieldDescriptor` of its `SectionDefinition`. Available properties: *DomInstance*, *Section*, *FieldDescriptor*. From DataMiner 10.4.2/10.5.0 onwards, fields that allow multiple values with an empty list as value, will be considered as missing fields.|
