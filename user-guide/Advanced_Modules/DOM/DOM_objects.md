...
From DataMiner 10.3.10/10.4.0 onwards, it's also possible to soft-delete a `GenericEnumEntry<T>` in a `Genericenum<T>` ona `GenericEnumFieldDescriptor`. When an entry is marked as soft-deleted, it will not be shown anymore in UI forms, except for an edit form where the value for the GenericEnum field is the soft-deleted value. It's not possible to create a `DomInstance` with a soft-deleted entry as value, or update a `DomInstance` enum field to a soft-deleted entry, this will return a [ValueForSoftDeletedFieldNotAllowed error](xref:DomInstance#errors). A `DomInstance` that has a soft-deleted entry as value is still allowed to exist and can still be updated.
