...
1. In the *DB* element for the indexing database, specify the custom prefix. Keep in mind that only regular alphanumeric characters are supported for the prefix, not symbols. The specified value should match across all the OpenSearch configurations in the *DB.xml*.
