...
From DataMiner 10.0.13 onwards, this tool allows you to back up and restore Elasticsearch database clusters. By default, it is available on a DMA in the folder `C:\Skyline DataMiner\Tools`. To get a full backup of the Elasticsearch cluster, you only need to execute the tool on one of the Elasticsearch nodes.
