...
Both *System Display* and *Element Display* are ActiveX controls. They are automatically downloaded and installed onto your computer the first time you try to open their user interface in a Microsoft Edge in IE compatibility mode.
...
> [!CAUTION]
> We strongly advise that you use HTTPS when you use DataMiner client applications over public internet. If you do not do so, all information - including logon credentials - is sent as plain, unencrypted text over the internet. See also: [Setting up HTTPS on a DMA](xref:Setting_up_HTTPS_on_a_DMA).
...
1. In the *Security Settings* dialog box, go to *ActiveX controls and plug-ins*.

1. Set *Download unsigned ActiveX controls* to “Prompt” and *Run ActiveX controls* to “Enable”.
...
1. In the *Security Settings* dialog box, go to *Scripting*.

1. Set *Active scripting* to “Enable”.
...
1. In the *Connections* tab, click *LAN settings*.

1. In the *Local Area Network (LAN) Settings* dialog box, go to the *Proxy server* section and click *Advanced*.

1. In the *Proxy Settings* dialog box, go to the *Exceptions* section and add the IP addresses of your DataMiner Agents to the list. Separate the addresses by means of semicolons (”;”).
...
1. In the left-hand pane of the *Manage Add-ons* window, set the *Show* selection box to “All add-ons”.

1. In the list of add-ons, go to the *Skyline Communications* section and select either “SLSystemDisplay Control” or “SLElementDisplay Control”.

1. In the bottom pane, click *More information*.

1. In the *More information* dialog box, click *Remove*.
...
1. In the *DataMiner Tools* section, click *Clean CAB Files* and then click *Run*.

1. In the *Clean CAB Files* window, click *Go*.
