...
| Indexing Engine | Dedicated indexing database, by default Elasticsearch, used to index information from the DataMiner System to allow rapid information retrieval. Required for certain DataMiner features, e.g. for DataMiner Job Manager.<br>See [Setting up an Elasticsearch database in a setup with storage per DMA](xref:Configuring_indexing_database_per_DMS). |
