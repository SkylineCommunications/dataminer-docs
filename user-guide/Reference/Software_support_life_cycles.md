...
| Use of JScript in QActions | End of Support | C# should be used instead. JScript in QActions cannot be used on Windows Server 2025. | End of Life to be confirmed |
...
|---------|---------|---------|---------|
| Support for Cassandra versions prior to 4.x | End of Support | Cassandra versions older than Cassandra 4.x are no longer supported on any operating system.| End of Life to be confirmed.|
...
| Two-site redundant indexing clusters | End of Support | For optimal redundancy, we recommend switching to [Storage as a Service](xref:STaaS). | End of Life to be confirmed. |
| Support for MySQL as local database | End of Engineering | Existing features will work, but some new features will require [Storage as a Service](xref:STaaS) (recommended) or [dedicated clustered storage](xref:Dedicated_clustered_storage). | End of Support as of DataMiner 10.6 (Q4 2025) |
| Support for Cassandra Single | End of Engineering | Support will end for setups where each DMA has its own Cassandra database. Instead we recommend switching to [STaaS](xref:STaaS). Though this is not recommended, you can also use [dedicated clustered storage](xref:Dedicated_clustered_storage) instead. | End of support as of DataMiner 10.7 (Q4 2026) |
| Support for Elasticsearch 6.8 | End of Engineering | As Elastic no longer supports Elasticsearch 6.8 (or lower), support for this will also end in DataMiner. We highly recommend switching to [STaaS](xref:STaaS). Though this is not recommended, you can also use [OpenSearch](xref:OpenSearch_database) instead. | End of support as of DataMiner 10.7 (SQ4 2026) |
