...
Do note that when using different logical operators in the Conditions expression, the order of operation follows the one of boolean algebra where the order of operations is (from highest priority to lowest) NOT then AND and finally OR.
Do also note that at the current time there is no support for the use of parenthesis. For more info see [Reference](xref:CI_Types1#conditionsoptions)
