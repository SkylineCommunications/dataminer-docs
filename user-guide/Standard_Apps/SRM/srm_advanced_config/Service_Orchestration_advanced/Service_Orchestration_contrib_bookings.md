...
1. Create the contributing service definition. This service definition will to define all functions that will be part of the contributing booking and how they will be interconnected. Extra configuration will also be required to indicate how bookings based on the service definition can generate a contributing resource.
   ...
   1. Add the [Contributing Configuration](xref:SRM_properties_Booking_Manager#contributing-configuration) property to the service definition, and set its value to a JSON string containing the following fields:

      - *LifeCycle*: *Locked* or *Unlocked*. See [Locked or unlocked](#locked-or-unlocked).

      - *ParentSystemFunction*: The GUID of the system function.

      - *ResourcePool*: The name of the resource pool where the contributing resource will be created.

      - *ReservationAppendixName*: Optional. The extra string that will be appended to the booking name, if any.

      - *OrchestrationTrigger*: *Main* or *Local*. See [Main or local](#main-or-local).
      ...
      - *Script*: Optional. Defines a custom Automation script that can be executed after the conversion of the booking to a contributing booking. This script is typically used to add capacities and capabilities on the contributing resources. <!-- [RN28243] [RN28961] -->

        The following placeholders can be used to pass information to the custom script:

        - [RESOURCEID]

        - [RESERVATIONID]

      - *PreRoll*: Optional. Pre-roll of the contributing booking. Only valid for Dijkstra use case

      - *PostRoll*: Optional. Post-roll of the contributing booking. Only valid for Dijkstra use case

      - *ReservationType*: Optional. Only used for Dijkstra use cases.

      - *LiteContributingResource*: Optional. See [Enabling lite contributing bookings](#enabling-lite-contributing-bookings).
...
<!-- TBD: This overlaps with http://localhost:8080/user-guide/Standard_Apps/SRM/Booking_Manager/Reference_Booking_Manager.html#contributing-configuration -->
