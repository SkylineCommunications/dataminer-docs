...
3. Optionally, you can further refine which bookings should be shown:
    ...
    - To filter the bookings, add a **ChildrenFilter** shape data field to the shape group and set its value to a booking filter, using the same filter format as is used when specifying a *ListView* filter. See [List view filters](Creating_a_list_view.md#list-view-filters).
