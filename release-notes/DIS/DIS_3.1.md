...
DIS now uses [Validator version 1.1.11](https://github.com/SkylineCommunications/Skyline.DataMiner.CICD.Validators/releases/tag/1.1.11).
DIS now uses [Skyline.DataMiner.XmlSchemas.Protocol version 1.0.7](https://github.com/SkylineCommunications/Skyline.DataMiner.XmlSchemas/releases/tag/1.0.7).

### Changes

#### Enhancements

##### SLDisDmaComm UnobservedTaskException handler [ID 41198]

In SLDisDmaComm, when there was an unobserved task exception event, the event handler code would log this and throw an exception to let it end the process. As this event could go off when e.g. publishing to a DaaS system this has now been updated to only log the unobserved exception.

#### Fixes

##### Inconsistent column width [ID 40737]

The table editor in DIS automatically detects and suggests a width for columns based on the column description and the values it will contain.
There was an issue with the calculation of the column width as it was based on the current culture. This has now been changed into using invariant culture so the suggested widths are consistent across culture setting on your development machine.

##### Fix for MIB generated description for column parameters [ID 41190]

A fix has been made in the parameter generator from MIBs so that the description of the table and its column parameters is now consistent.
