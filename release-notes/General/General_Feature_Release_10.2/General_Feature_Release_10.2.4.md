...
# General Feature Release 10.2.4 [ARCHIVED]
...
#### SNMP forwarding will now take into account alarm storms when resending alarms \[ID 32543\]
...
#### SLSSH: Enhanced cypher and key exchange algorithm support \[ID 32664\]
...
#### Visual Overview: Specifying a remote debugging port for Chromium and Edge browser engines \[ID 31792\]
...
#### DataMiner Cube - Users/Groups: Permissions needed to edit group membership \[ID 32456\]
...
#### Visual Overview: Shapes that cannot be clicked and had their text trimmed will show a tooltip with the full shape text \[ID 32463\]
...
#### New setting to enable the cache of the Chromium browser engine \[ID 32534\]
...
#### Visual Overview: Linking a shape to an alarm filter with a System Type filter context \[ID 32548\]
...
#### DataMiner Cube - Data Display: Row filter will now be shown when opening the alarm template, trend template or information template for a particular parameter table row \[ID 32555\]
...
#### DataMiner Cube - Alarm Console: Clearer summary alarm values in case of an alarm storm \[ID 32608\]
...
#### Trending: Refresh rate of trend graphs can now be configured \[ID 32715\]
...
#### Dashboards app - Node edge graph: Option to visualize the direction of the edges \[ID 32519\]
...
#### Dashboards app - Parameter feed: 'Selected only' toggle button has been removed \[ID 32541\]
...
#### Dashboards app: Specifying data input for an EPM feed in a URL using 'epm-selections' instead of 'cpes' \[ID 32594\]
...
#### Dashboards app: Default index filter for parameter feed component \[ID 32595\]
...
#### Dashboards app: 'Write' filter removed from Data \> All available data \> Parameters \[ID 32643\]
...
#### Dashboards app: User groups can now be selected in dashboard security \[ID 32681\]
...
#### Jobs app: Delete button will only be visible to users who have been granted permission to delete jobs \[ID 32507\]

From now on, the *Delete* button will only be visible to users who have been granted permission to delete jobs.

### DMS Service & Resource Management

#### Service & Resource Management: Updated exceptions thrown when the time range of a child ReservationInstance or child ReservationDefinition is invalid \[ID 32581\]
...
#### Security enhancements \[ID 32566\]

A number of security enhancements have been made.

#### DataMiner is now able to connect to Cassandra databases that require TLS 1.2 \[ID 31852\]

DataMiner is now able to connect to Cassandra databases that require TLS 1.2.

#### Cassandra will no longer store ArrowWindowRecords and PatternMatchOccurrenceRecords \[ID 31944\]
...
#### SLElement: Enhanced performance when resolving table relationships \[ID 32176\]

Due to a number of enhancements, overall performance of SLElement has increased when resolving table relationships.

#### Filtered subscriptions will now be processed similarly to non-filtered subscriptions \[ID 32399\]
...
#### DataMiner backups will now by default also include SSL certificates and EPMConfig.xml files \[ID 32504\]
...
#### DataMiner Cube - Correlation: Using hidden elements when creating correlation rules \[ID 32510\]
...
#### Enhanced performance when stopping SNMP elements \[ID 32523\]
...
#### DataMiner Cube - Spectrum analysis: Additional logging to help pinpoint monitor failures \[ID 32605\]
...
#### DataMiner Cube: Information templates will only show the read parameter in case of a toggle button \[ID 32610\]
...
#### Video thumbnails: New options for HTML5 video player & VLC player \[ID 32626\]
...
#### Enhanced performance of the interprocess communication between SLDataGateway and SLAnalytics \[ID 32653\] \[ID 32709\]
...
#### Server processes will now use InvariantCulture \[ID 32665\] \[ID 32912\]
...
#### Backups will now by default also include the SLAnalytics configuration \[ID 32699\]
...
#### Size of SLMessageBroker.txt log file is now limited to 3MB \[ID 32863\]

The size of the SLMessageBroker.txt log file is now limited to 3MB.

#### NATS: Enhanced handling of serialization issues \[ID 32883\]
...
#### Cassandra: Problem when reading DVEElementInfo rows that contained NULL values \[ID 32357\]

Up to now, DataMiner could throw an exception when reading a DVEElementInfo row that contained NULL values.

#### SNMP forwarding: Problem when deleting an SNMP manager and immediately creating an identical one \[ID 32406\]
...
#### DataMiner Cube - Data Display: Not possible to open the alarm template or trend template of a parameter that was not trended \[ID 32449\]
...
#### DataMiner Cube - Profiles app: Problem when trying to retrieve mediation snippets and service profiles on a system without ElasticSearch database \[ID 32488\] \[ID 32539\]
...
#### Stopping an SNMPv3 element could have a negative impact on the overall performance of other SNMPv3 elements \[ID 32498\]
...
#### Problem when trying to delete a protocol \[ID 32522\]

In some cases, an error could occur when you tried to delete a protocol.

#### Dashboards app: Not possible to share a dashboard immediately after connecting a DMA to the cloud \[ID 32547\]
...
#### Dashboards: Problem when sharing a dashboard created in an earlier version of the Dashboards app \[ID 32551\]
...
#### Web services API: Problem with internal GQI calls when web sockets were disabled \[ID 32570\]
...
#### Elasticsearch: Problem when using a filter that checked whether a string was empty \[ID 32586\]
...
#### Elasticsearch: Page would return empty when requesting the next page of naturally sorted data \[ID 32596\]
...
#### Jobs app: No 'loading' indication when job sections were being loaded \[ID 32616\]
...
#### Web services API: Problem with GetServiceTemplate \[ID 32625\]
...
#### SLNet would leak memory whenever a Cube session was closed or disconnected \[ID 32649\]

Whenever a Cube session was closed or disconnected, SLNet would leak memory.

#### Miscellaneous small fixes \[ID 32655\]

A number of miscellaneous, small fixes have been made.

#### Dashboards app - Parameter feed: Problem selecting the element again after clicking 'Clear all' \[ID 32661\]
...
#### Migration to Cassandra cluster: Alarms and information events would not get migrated \[ID 32666\]
...
#### DataMiner Cube - Profiles app: Problem when modifying a profile instance \[ID 32688\]
...
#### DataMiner Cube - Alarm templates: Table conditions would no longer be saved correctly \[ID 32691\]
...
#### Dashboards: Missing sidebar when navigating to a dashboard URL for edit mode without having edit access \[ID 32706\]
...
#### SLAnalytics: Problem when stopping SLAnalytics features \[ID 32734\]

In some cases, an error could occur in the SLAnalytics process when SLAnalytics features were stopped.

#### SLAnalytics: Problem when checking the status of the Elasticsearch database \[ID 32753\]
...
#### Alarms and information events not migrated during migration to Cassandra Cluster \[ID 32755\]
...
#### DataMiner Cube - Asset Manager: UI could become unresponsive when the database configuration was being retrieved \[ID 32766\]
...
#### SLAnalytics: Unneeded error was thrown when an upgrade action tried to remove the pattern match occurrences table from a non-existing Elasticsearch database \[ID 32772\]
...
#### SLAnalytics - Pattern matching: Disabling the monitoring of a pattern would not be applied immediately [ID 32792]
