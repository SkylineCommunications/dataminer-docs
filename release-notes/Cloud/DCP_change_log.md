...
#### 5 December 2022 - New feature - Remote Log Collection 

A new DataMiner Cloud Pack 2.8.2 has been released which enables [Remote Log Collection](xref:RemoteLogCollection#RemoteLogCollection). The DataMiner Cloud Pack can be found on [DataMiner Dojo](https://community.dataminer.services/downloads/).

[Remote Log Collection](xref:RemoteLogCollection#RemoteLogCollection) allows our experts to collect DataMiner log and memory dump files independently, increasing efficiency of investigations.

The packages generated by the Log Collector tool are automatically transferred to secure storage in the cloud, so that our experts have immediate access to the packages.

Note that from DataMiner CloudGateway 2.10.0 onwards, included in DataMiner Cloud Pack 2.8.2, it is required for the internal network to allow [HTTP(S) traffic via port tcp/5100](xref:Configuring_the_IP_network_ports#overview-of-ip-ports-used-in-a-dms). This is used as a cloud endpoint for other [DataMiner Extension Modules](xref:DataMinerExtensionModules#dataminer-extension-modules-dxms), in this case it is required for the [Remote Log Collection](xref:RemoteLogCollection#RemoteLogCollection) feature. For more information about disabling or configuring another port for the cloud endpoint, please refer to [Custom cloud endpoint configuration](xref:Custom_cloud_endpoint_configuration).
