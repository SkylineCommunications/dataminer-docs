...
Note that from DataMiner CloudGateway 2.9.6, included in DataMiner Cloud Pack 2.9.0, it is required for the internal network to allow [HTTP(S) traffic via port tcp/5100](xref:Configuring_the_IP_network_ports#overview-of-ip-ports-used-in-a-dms) used as a cloud endpoint for other [DataMiner Extension Modules](xref:DataMinerExtensionModules#dataminer-extension-modules-dxms), in this case required for the [Remote Log Collection](xref:RemoteLogCollection#RemoteLogCollection) feature. More information about disabling or configuring another port for the cloud endpoint can be found on [Custom cloud endpoint configuration](xref:Custom_cloud_endpoint_configuration).
