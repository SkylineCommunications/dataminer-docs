...
- **Add Domain modal**: A setup wizard to define a new PTP domain and assign initial devices and views (only elements supported by the solution are listed). If no domains exist yet (such as during first-time onboarding), this wizard is automatically presented as an onboarding overlay.
