...
- An AMPP Control **API key** generated using the AMPP Identity Manager.
  ...
  > [!TIP]
  > Starting with version 1.1.8, the installed AMPP Manager connector can also report **Subscriptions and Billing** information. To enable these features, you need an additional Application Client API Key of type *subscriptions.admin*, *cluster.admin* role, and allowed scopes for *cluster platform*. For detailed setup instructions, refer to the AMPP documentation hub.
  >
...
2. Open the *GV AMPP Manager* app.
3. In the app, click the cogwheel ![cogwheel](~/dataminer/images/GV_AMPP_Manager_Configuration_page.png) icon in the sidebar to open the *Setup* page.
...
4. Click the *Configure* button to launch the configuration script.

   ![AMPP Setup Script](~/dataminer/images/GVAMPP_Setup_Script.png)
5. In the *Dialog* window, provide the following information:
