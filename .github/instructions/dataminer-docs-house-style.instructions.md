...
- Only use one of the following alert types: `> [!NOTE]`, `> [!TIP]`, `> [!IMPORTANT]`, `> [!CAUTION]`, and `> [!WARNING]`. No other alert type (e.g., `[!ALERT]`) is supported. Before adding or editing an alert, verify the type against `/contributing/CTB_Markdown_Syntax.md`.
