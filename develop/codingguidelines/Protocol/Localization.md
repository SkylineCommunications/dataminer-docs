...
For the same reason, when a command containing a numeric value as string is created from a QAction, conversion using culture info is necessary as otherwise the command string could be formatted incorrectly. For example, Convert.ToString(signalStrength) could result in "10,05" where the device would expect "10.05". Therefore, additional culture info is necessary when the conversion is done:
