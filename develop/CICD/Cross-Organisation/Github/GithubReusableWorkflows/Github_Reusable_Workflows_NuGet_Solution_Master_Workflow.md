...
The NuGet Solution Master Workflow is designed to run on repositories that hold the [DataMiner NuGet Package Solution](xref:Automation_scripts_as_a_Visual_Studio_solution) provided by the DIS plugin in Visual Studio or from [Skyline.DataMiner.VisualStudioTemplates](https://www.nuget.org/packages/Skyline.DataMiner.VisualStudioTemplates#readme-body-tab).
...
To use this workflow, your Visual Studio solution must have all projects in SDK-Style. If you are migrating an existing library, you will need to convert it into an SDK Style. The easiest way to do this is to create a new solution with the template and then move all content from the old solution. While there are ways to edit an existing solution to change all projects to SDK, it is usually easier and less error-prone to generate that part and just reinstall all necessary NuGet packages and move all project content.
...
This job will validate in 4 important ways:
...
If the library passes these checks, the job will archive and provide the created NuGet package as an artifact and attempt to sign it with the next job.
...
If signing is successful the next job will attempt to push the package to nuget.org.
...
> A release cycle can also release a pre-release version of a NuGet. To do that, simply tag with format A.B.C-text. (e.g. 1.0.1-AlphaOne).
...
2. use code: certutil -encode infile outfile
3. open the outfile with a txt editor and copy the string content.
4. Paste that contents into an Action Secret in github called PFX
5. Add a second Action Secret in github called PFXPASSWORD holding the password of the pfx.
