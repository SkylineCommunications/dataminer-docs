...
> [!TIP]
> The following wrapper workflows now delegate to the Master Workflow internally and are still available for backward compatibility:
>
...
The main quality gate job. Runs the build, tests, and static code analysis. See [CI job details](#ci-job-details) below.

### Sign NuGet packages

Only runs during a release cycle (tag) and when NuGet packages were produced by the CI job. Signs `.nupkg` files using `dotnet sign` with Azure Key Vault credentials. If signing credentials are not available, the unsigned packages are passed through.
...
Only runs during a release cycle (tag). Pushes the signed NuGet packages to the configured destination:
...
Only runs during a release cycle (tag) and when DataMiner packages (`.dmapp`/`.dmtest`) were produced by the CI job. This job:

1. Signs the DataMiner packages using Azure Key Vault credentials.
1. Generates an SBOM (Software Bill of Materials) for each package.
1. Extracts release notes from the GitHub release (if available) or falls back to the commit message.
1. Publishes the packages to the [DataMiner Catalog](https://catalog.dataminer.services/).
...
Before proceeding with the build, the workflow validates:

- That the SonarCloud project name corresponds to an existing project.
- That `SONAR_TOKEN` is available when SonarCloud analysis is required.
- That `DATAMINER_TOKEN` is available when a release tag triggers the workflow and DataMiner packages need to be published.

### Build

Compiles the Visual Studio solution after restoring all NuGet packages. The version is determined as follows:
...
Searches for test projects in the solution and runs all unit tests found. The workflow supports two test runner modes:
...
Performs static code analysis using [SonarCloud](https://www.sonarsource.com/products/sonarcloud/). This checks for common errors and bugs in C# code, tracks code coverage, and enforces clean code guidelines. The results are evaluated against the SonarCloud quality gate.
...
Combines the results of all previous steps into a single pass/fail result. The workflow fails if:
