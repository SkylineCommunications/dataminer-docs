...
Only when the actions above and "Artifact Creation" job have been successful, will the "Artifact Registration and Upload" job be executed. This job will upload an artifact (.dmprotocol) based on the Connector solution, with the following steps:
...
This job runs in parallel and will create the .dmprotocol package. This will be provided as an artifact, downloadable directly from the run and can be used for manual testing.
