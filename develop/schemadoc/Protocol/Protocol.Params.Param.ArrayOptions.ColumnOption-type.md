...
> [!NOTE]
>
> - The displaykey column is there for the end-user only. Therefore, the column has no reason to live within SLProtocol which will fully rely on the primary keys instead for its logic. It only lives in SLElement which is responsible to build the value. This has following implications:
>    - The displaykey column should alway be the last column defined in the table. Otherwise, since the displaykey column does not exist within SLProtocol, all subsequent columns would have a confusing shifted IDX.
