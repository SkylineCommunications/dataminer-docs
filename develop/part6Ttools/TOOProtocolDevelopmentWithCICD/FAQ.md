...
2. Click the *Create Repository* button. Provide the name of the vendor and data source and click the *Create* button.

    > [!NOTE]
    > The name of the data source should start with the name of the vendor.

    ![](~/develop/images/SLCSERepoManager_CreateRepo.png)
    <br>Figure 20: SLC SE Repository Manager: Creating a repository
...
4. Create a new DataMiner protocol Visual Studio solution in the local repository folder.

    1. Start Visual Studio and go to *File* > *New* > *DataMiner Protocol Solution*.

    2. Specify the protocol name.

    3. For the target location, specify the local Git repository folder that you created earlier.

    4. Click *OK*.

    ![](~/develop/images/VS_NewProtocolSolution.png)
	<br>Figure 21: New DataMiner Protocol Solution window

5. Specify the vendor in the protocol.xml file.

6. Open your local repository using SourceTree. You will see that the repository already has a 1.0.0.X branch. Now create a new local branch with the following name: *gerrit/DCP\<TaskID>_\<Description>* (e.g. DCP1234_FixLayout).

	![](~/develop/images/SourceTree_BranchCreation.png)
	<br>Figure 22: Creating a branch in SourceTree
...
7. Perform an initial commit: Stage all the new files of the newly created Visual Studio solution and provide a commit message, then click *Commit*.

	![](~/develop/images/SourceTree_InitialCommit.png)
	<br>Figure 23: Initial commit in SourceTree

    A new commit will be added to your new branch.

	![](~/develop/images/SourceTree_CommitAddedToBranch.png)
	<br>Figure 24: New commit added to new branch in SourceTree

8. Implement your work, performing as many commits as you want.
...
> [!TIP]
> See also:
> Learning path "System Dev - CI/CD, GIT, Gerrit" on LinkedIn Learning, Section "Tutorial Protocol Development":  CI/CD HowTo - Start a new Protocol Development.
...
2. Navigate to the vendor in the tree control and verify whether your data source is already listed under that vendor.
...
3. If the data source you will work on was not yet listed, click the *Create Repository* button, specify the name of the vendor and data source and click *Create*.

	![](~/develop/images/SLCSERepoManager_CreateRepoExisting.png)
	<br>Figure 25: SLC SE Repository Manager: Creating a repository

4. A confirmation message will be displayed, stating that the protocol was found on SVN and asking if you want to convert this into a Git repository. Click *Yes* to confirm.

    The SLC SE Repository Manager tool will automatically clone the repository.

5. Open the repository in SourceTree.

    > [!NOTE]
    > Always make sure your local repository is up to date.

	![](~/develop/images/SourceTree_Repo.png)
	<br>Figure 26: Repository in SourceTree
...
6. Check out the range you are going to work on. (By default, range 1.0.0.X is checked out).

	![](~/develop/images/SourceTree_RangeCheckout.png)
	<br>Figure 27: Checking out a range in SourceTree

7. Navigate to the repository folder in File Explorer and verify if the previous version already contains a DataMiner protocol Visual Studio solution. If it does, skip to step 11.

8. If the previous version contained no solution yet, in the *fromSvn* folder, you should find a protocol.xml file. Open this file in Visual Studio and convert it to a Visual Studio solution using DIS.

    1. In Visual Studio, go to *DIS* > *Protocol* > *Convert to Solution*.

		![](~/develop/images/VS_ConvertToSolution.png)
		<br>Figure 28: Convert to Solution in DIS

    2. In the pop-up window, specify the repository path as the target location and click *Convert*.

		![](~/develop/images/ConvertToSolution.png)
		<br>Figure 29: Convert Protocol to Solution window

9. In SourceTree, commit the solution you created in the previous step.

	![](~/develop/images/SourceTree_CommitSolution.png)
	<br>Figure 30: Committing a solution in SourceTree

10. In Sourcetree, push the commit to the server

	![](~/develop/images/SourceTree_PushCommit.png)
	<br>Figure 31: Pushing a commit to the server in SourceTree.

11. Create a new local branch with the following name: *gerrit/DCP\<TaskID>_\<Description>* (e.g. DCP1234_FixLayout).

	![](~/develop/images/SourceTree_NewLocalBranch.png)
	<br>Figure 32: Creating a new local branch in SourceTree

    > [!NOTE]
    > Normally, Sourcetree should automatically switch to this branch. If for some reason this does not happen, switch to this branch manually.

12. Start development on the new branch.

> [!TIP]
> See also:
> Learning path "System Dev - CI/CD, GIT, Gerrit" on LinkedIn Learning, Section "Tutorial Protocol Development":  CI/CD HowTo - Start a protocol from an existing one on SVN
...
2. Create the new range branch, for example 2.0.1.X.

	![](~/develop/images/SourceTree_NewRangeBranch.png)
	<br>Figure 33: Creating a new range branch in SourceTree

3. Push this branch to the server so the Git repository hosted by Gerrit is aware of this branch.

	![](~/develop/images/SourceTree_PushNewBranch.png)
	<br>Figure 34: Pushing a new branch to the server in SourceTree

4. Create your task branch based on the newly created range branch and start development.
...
![](~/develop/images/SourceTree_PushToServer.png)
<br>Figure 35: Pushing work to the remote repository using SourceTree
...
![](~/develop/images/SLCSERepoManager_PushWIP.png)
<br>Figure 36: Pushing work into Gerrit as work in progress
...
3. Commit the checklist to your local Git repository using SourceTree.

4. Push the commit. As the checklist itself does not require any code review, you can push the checklist immediately to the remote Git repository hosted by Gerrit using SourceTree.

> [!TIP]
> See also:
> Learning path "System Dev - CI/CD, GIT, Gerrit" on LinkedIn Learning, Section "Tutorial Protocol Development":  CI/CD HowTo - Development Checklist and DriverQA.
...
1. In the commit overview in SourceTree, select the commit for which you would like to create a tag, right-click, and select *Tag* in the context menu.
	...
	![](~/develop/images/SourceTree_CreateTag.png)
	<br>Figure 37: Creating a tag for a commit in SourceTree

2. Specify the tag name (e.g. "1.0.0.9"), ensure that the *Specified commit* option is selected and select the check box *Push tag*, so that the tag also gets pushed to the remote repository.

	![](~/develop/images/SourceTree_CreateTagWindow.png)
	<br>Figure 38: Creating a tag for a commit in SourceTree

3. Click *Add Tag*. This will make Jenkins perform a final execution of the pipeline for this newly created protocol version. It will detect that you provided a tag and will therefore publish this version on SVN.

> [!TIP]
> See also:
> Learning path "System Dev - CI/CD, GIT, Gerrit" on LinkedIn Learning, Section "Tutorial Protocol Development":  CI/CD HowTo - How to release a new version.
...
1. The first step is to select your latest commit in the range you were working on (in this example 1.0.0.X) and select the branch icon in SourceTree to create the a new branch for the new major range.
	...
	![](~/develop/images/SourceTree_BranchIcon.png)
	<br>Figure 39: The branch icon in SourceTree

2. In the pop-up window, provide the name of the new branch (in this example, 1.0.1.X) and click *Create Branch*.

	![](~/develop/images/SourceTree_CreateBranch.png)
	<br>Figure 40: Creating a branch in SourceTree

    You should now see that the new range has been created locally:

	![](~/develop/images/SourceTree_BranchCreated.png)
	<br>Figure 41: New local range

3. Now you also need to push this new range to the remote. To do this, click the *Push* button and, in the pop-up window, select the check box for the new branch and click *Push*.

	![](~/develop/images/SourceTree_PushRange.png)
	<br>Figure 42: Pushing a new range in SourceTree

    You should now see that the new range has been created on the remote as well:

	![](~/develop/images/NewRangeCreated.png)
	<br>Figure 43: New range on the remote
...
2. Then select the commit that represents the latest valid commit. This should be the commit that represents the tag of the previous version (in this example, the commit for tag 1.0.0.2).

3. After selecting the commit in the graph overview, right-click and select *Reset current branch to this commit* in the context menu.

	![](~/develop/images/SourceTree_CurrentBranchToCommit.png)
	<br>Figure 44: “Reset current branch to this commit” option

4. In the pop-up window, select the mode *Hard* (this will discard all working copy changes) and click *OK*.

	![](~/develop/images/SourceTree_ResetToCommit.png)
	<br>Figure 45: Hard reset mode selection

5. In the pop-up window asking for confirmation, click *Yes*.

	![](~/develop/images/DestructiveOperation.png)
	<br>Figure 46: Confirmation window for hard reset mode
	...
	![](~/develop/images/PointyBranch.png)
	<br>Figure 47: 1.0.0.X branch is pointing to the commit for the 1.0.0.2 tag
...
6. To push, click the *Push* button and make sure the *Force Push* check box is selected. Then click *Push*.

	![](~/develop/images/ForcePush.png)
	<br>Figure 48: “Force Push” option

7. In the confirmation box, click *Yes*.

	![](~/develop/images/ConfirmForcePush.png)
	<br>Figure 49: Force Push confirmation box

    At this point, you should see that head of the origin/1.0.0.X branch is now also pointing to the desired commit.

	![](~/develop/images/GoodPointyBranch.png)
	<br>Figure 50: 1.0.0.X branch pointing to correct commit
...
![](~/develop/images/ChangeInTheWrongRange.png)
<br>Figure 51: Changes pushed to 1.0.0.X range

1. Perform a checkout of the branch you want to work on (in this example 1.0.1.X) and make sure this is your active branch.

2. Now you need to apply the commits on the correct 1.0.1.X branch in the same order as you applied them on the 1.0.0.X branch. To do this, you will apply cherry picking. Select the first commit (in the example *feature 1*) right-click and select *Cherry Pick* in the context menu:

	![](~/develop/images/CherryPick.png)
	<br>Figure 52: Cherry pick a commit

3. In the pop-up window, click *OK*.

	![](~/develop/images/ConfirmCherryPicking.png)
	<br>Figure 53: Confirm cherry picking

4. Repeat this for all the commits you want to have on the current branch.

	![](~/develop/images/RepeatForBranch.png)
	<br>Figure 54: Repeat for the commits on the current branch

5. Push the cherry-picked commits to the remote repository.

	![](~/develop/images/PushingCherries.png)
	<br>Figure 55: Push the cherry-picked commits to the remote repository

	![](~/develop/images/PushedCherries.png)
	<br>Figure 56: The cherry-picked commits in the remote repository

6. Now you will still need to remove the commits on the branch you were originally working on (in this example, 1.0.0.X). To do this, check out the 1.0.0.X branch to make it the active one.
...
7. To remove the commits from the 1.0.0.X branch, make sure this range is the one that is currently checked out. Then select the commit that represents the latest valid commit. This should be the commit that represents the tag of the previous version.

8. When you have selected the commit in the graph overview, right-click and select *Reset current branch to this commit* in the context menu.

9. In the pop-up window, select the mode *Hard* (this will discard all working copy changes) and click *OK*.

	![](~/develop/images/SourceTree_ResetToCommit00077.png)
	<br>Figure 57: Hard reset mode selection

10. In the pop-up window asking for confirmation, click *Yes*.

	![](~/develop/images/DestructiveOperation00078.png)
	<br>Figure 58: Confirmation window for hard reset mode

11. At this point, you should see that the head of your local 1.0.0.X branch is pointing to the commit for the 1.0.0.2 tag. However, the head of origin/1.0.0.X is not. This is because you first need to push. To push, select the *Push* button and make sure the *Force Push* check box is checked. Then click *Push*.

12. In the confirmation box, click *Yes* to confirm.

	![](~/develop/images/ConfirmForcePush00079.png)
	<br>Figure 59: Force Push confirmation box
...
![](~/develop/images/FixedBranch.png)
<br>Figure 60: Fix in range 1.0.1.X

1. First, perform a checkout of the branch you want to include the commit (in this example 1.0.2.X).

2. Then select the commit you want to cherry pick, right-click and select *Cherry Pick* in the context menu.

	![](~/develop/images/CherryPickFix.png)
	<br>Figure 61: Cherry pick a fix

3. In the pop-up window, click *OK*.

	![](~/develop/images/ConfirmCherryPicking00080.png)
	<br>Figure 62: Confirm cherry picking

Now you will see that the change has been committed to the 1.0.2.X branch.

![](~/develop/images/FixCommittedToBranch.png)
<br>Figure 63: Fix in range 1.0.2.X
...
1. Open the SLC SE Repository Manager tool, select the protocol you want to push for review in the tree control and select the *Push Repository* button.
	...
	![](~/develop/images/SLCSERepoManager_PushRepo.png)
	<br>Figure 64: Pushing a repository in SLC SE Repository Manager

2. Select the branch you will eventually merge your work into and click the *Push for Gerrit Review* button.

	![](~/develop/images/SLCSERepoManager_PushForReview.png)
	<br>Figure 65: Push for Gerrit review
...
3. In Gerrit, select the new entry in the review dashboard in the *Outgoing reviews* section.

    This will open a new page for this item.

	![](~/develop/images/GerritOutgoingReviews.png)
	<br>Figure 66: Outgoing reviews in Gerrit

4. In the *Assignee* field, specify the name of the person who should perform the review.

	![](~/develop/images/GerritSpecifyAssignee.png)
	<br>Figure 67: Specifying the name of the reviewer in Gerrit

    The reviewer will now be notified by email.

> [!TIP]
> See also:
> Learning path "System Dev - CI/CD, GIT, Gerrit" on LinkedIn Learning, Section "Tutorial Gerrit Code Review":  CI/CD HowTo - Assign a Gerrit Code Review.
...
1. In Gerrit, on the review dashboard page, select the new entry in the *Incoming reviews* section.
	...
	![](~/develop/images/GerritIncomingReviews.png)
	<br>Figure 68: Incoming reviews in Gerrit

    On the review page, you will see an overview of all the files that have been updated in the Files section.

	![](~/develop/images/GerritUpdatedFiles.png)
	<br>Figure 69: Overview of updated files in Gerrit
...
2. Click on the files to see the details of the changes to these files and review those.

	![](~/develop/images/GerritCodeReview.png)
	<br>Figure 70: Code review in Gerrit

3. Provide review comments where necessary by double-clicking the desired location in the code and selecting *Provide comment* (or use the c shortcut). When you have entered a comment, click *save*.

4. When you have reviewed all changed files, on the review page, click the reply button and provide the necessary feedback.
	...
	![](~/develop/images/GerritCodeReviewReply.png)
	<br>Figure 71: Code review in Gerrit
...
5. Click *Send*. Now the developer will receive an email about this review.

> [!TIP]
> See also:
> Learning path "System Dev - CI/CD, GIT, Gerrit" on LinkedIn Learning, Section "Tutorial Gerrit Code Review":  CI/CD HowTo – Perform a Gerrit Code Review and CI/CD HowTo – Handle fixes sent to Gerrit.
...
1. In Gerrit, on the review dashboard page, click the reviewed item.
	...
	![](~/develop/images/GerritReviewedItem.png)
	<br>Figure 72: Reviewed item in Gerrit

2. In the files section, you can see in the *Comments* column which files have comments. Select a file to see all the comments for that file.

	![](~/develop/images/GerritCommentsOnFile.png)
	<br>Figure 73: Comments on a file in Gerrit

	![](~/develop/images/GerritCommentsOnFile2.png)
	<br>Figure 74: Comments on a file in Gerrit

3. In SourceTree, make sure you have checked out the corresponding branch and make sure it is up to date. In case you no longer have this branch locally, check it out from the remote repository.

4. Implement the required changes to address all code review remarks.

5. Commit your new changes in SourceTree.

6. Using the SLC SE Repository manager tool, send the new commit for code review.

	![](~/develop/images/SLCSERepoManager_PushForReview2.png)
	<br>Figure 75: Sending a commit for code review in SLC SE Repository Manager
...
1. In Gerrit, on the review page of your item you will now see a *Ready to submit* notification. Click the *SUBMIT* button.
	...
	![](~/develop/images/GerritReadyToSubmit.png)
	<br>Figure 76: Ready to submit notification in Gerrit

    Now it should show a *Merged* notification.

	![](~/develop/images/GerritMerged.png)
	<br>Figure 77: Merged notification in Gerrit

    If you now fetch the branch you merged into, you will see that the origin now points to your merged commit.

	![](~/develop/images/SourceTree_MergedCommit.png)
	<br>Figure 78: Merged commit in branch

2. In SourceTree, you can now delete your development branch:
    ...
    2. Select the branch to delete, right-click, and select *Delete* in the context menu.

		![](~/develop/images/SourceTree_DeleteBranch.png)
		<br>Figure 79: Deleting a development branch in SourceTree

		This will now show a pop-up window as illustrated below.

		![](~/develop/images/ConfirmBranchDeletion.png)
		<br>Figure 80: Confirm Branch Deletion window

3. Click *OK* to remove the branch.

4. Also remove the development branch in the remote repository:
	...
	![](~/develop/images/SourceTree_RemoveDevelopmentBranch.png)
	<br>Figure 81: Removing a development branch in the remote repository

5. At this point, you will typically will want to create a tag to define a new version of the protocol. For more information on how to do this, refer to [I finished development. How do I create a tag for the new version?](#i-finished-development-how-do-i-create-a-tag-for-the-new-version).

> [!TIP]
> See also:
> Learning path "System Dev - CI/CD, GIT, Gerrit" on LinkedIn Learning, Section "Tutorial Gerrit Code Review":  CI/CD HowTo – Finalize a Gerrit Code Review
