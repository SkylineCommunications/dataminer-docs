...
- **delay**: With this option, you can specify the delay in ms between two consecutive ping packets. This should be used when the device doesn't respond to all ping requests when they're sent without any delay.
  *Feature introduced in DataMiner 10.2.11 (RN 34463).*

  > [!NOTE]
  > - This option is only relevant when amountPackets or amountPacketsPID are used, which are currently only supported in conjunction with the threadPool option. When no threadPool is used, only one ping request will be sent.
...
- **delayPID**: Instead of specifying a fixed delay value ("delay=x"), it is also possible to specify a dynamic value stored in a parameter. Note that if you specify both a fixed and a dynamic value, the latter will take precedence.
  ...
  The value in the referred parameters must not be 0 or uninitialized. If it is, by default 0, the hard-coded value on the timer, or the last valid value will be used. The referred parameters must be of numeric type.
