...
> [!NOTE]
> If the QAction has no triggers, it should have the precompile option.
> QActions with the precompile option are compiled in the order in which these appear in the protocol XML. Therefore, if you have a precompile QAction that references another precompile QAction, the referenced precompile QAction should be defined earlier in the protocol XML document.
> By default, QActions are compiled in Release mode. To let it compile in Debug mode, the [debug](xref:Protocol.QActions.QAction-options#debug) option can be used.
