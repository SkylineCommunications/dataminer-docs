...
> [!IMPORTANT]
> Cleanup configuration is mandatory for support of a DMS Cluster with Failover setup. If the agent with the element that creates the monitors remains running, but the agent of the destination element goes into failover then you need to setup a cleanup delay or else all subscriptions for that destination element will be removed.
