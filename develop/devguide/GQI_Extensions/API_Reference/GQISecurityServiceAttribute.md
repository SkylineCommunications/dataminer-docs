---
uid: GQI_GQISecurityServiceAttribute
---
...
|--|--|
| `GQISecurityServiceAttribute()` | Registers and injects the service by its concrete type. |
| `GQISecurityServiceAttribute(Type serviceType)` | Registers and injects the service by the specified contract type. The decorated class must be assignable to this type. |
...
| IdleExpirationMinutes | `int` | Inherited from [GQIServiceAttribute](xref:GQI_GQIServiceAttribute). The default value is `60`. |
