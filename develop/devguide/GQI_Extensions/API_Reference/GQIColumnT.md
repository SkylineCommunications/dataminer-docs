...
|--|--|--|--|
| DiscreteOptions | [GQIDiscreteOptions\<T\>](xref:GQI_GQIDiscreteOptionsT) | The discrete value options configured for this column, or `null` if no discrete values are defined. | DataMiner Web 10.5.0 [CU17]/10.6.0 [CU5]/10.6.8<!-- RN 45380 --> |

### Derived types

Each derived type has two constructors: one that accepts only a name (backward compatible), and one that also accepts a [GQIDiscreteOptions\<T\>](xref:GQI_GQIDiscreteOptionsT) parameter (available from DataMiner Web 10.5.0 [CU17]/10.6.0 [CU5]/10.6.8 onwards<!-- RN 45380 -->).
