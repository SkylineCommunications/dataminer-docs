...
|Skyline.DataMiner.Dev.Utils.*     |These NuGet packages are the so-called *Custom Dev Packs*. Refer to [Custom Dev Packs](xref:TOODataMinerDevPackages#custom-dev-packs) for more information.       |
...
|Skyline.DataMiner.Core.DataMinerSystem.*     |These NuGet packages form the class library to interact with a DataMiner System from a connector or an automation script. Refer to [About the DataMinerSystem library](xref:ClassLibraryIntroduction) for more information.       |
