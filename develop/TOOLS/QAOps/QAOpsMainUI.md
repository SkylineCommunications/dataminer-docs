...
- Use *Fit*, *Reset pins*, *Refresh*, and *Reconnect* to control the graph, and the *Motion* dropdown to tune animations.
...
With the toggle next to the *Motion* dropdown, you can switch from the topology graph to an accessible **topology table** that lists the same entities with their status, metric, and relationship count.
...
At the top, select a *Global category*, *Configuration*, and *Test suite*. The page then shows:

- **Test runs**: All runs of the selected suite, with status, run ID, target, and start time. Use the search box, *Status* filter, and *Columns* selector to narrow the list down.
...
With the *Side by side*/*Stacked* toggle, you can arrange these panels to your liking, and with *Share* you can copy a link to the current selection.
...
The cards on the *Tokens* page show each token's expiration date, creation date, last-used date, and usage count. Tokens that are (almost) expired or that have exhausted their usage are highlighted. With the *Delete* button, you can remove a token you no longer need.
...
To edit a configuration, click the **gear icon** on its card. The *Edit Configuration* dialog opens with the current settings pre-filled. Adjust the settings and click *Save Configuration*; the *Status* box at the bottom of the dialog confirms the result.
...
To change a value, click the pencil icon next to it, enter the new value, and confirm with the **✓** button. This pushes your change through the QAOps system. The *New Configuration* button below the table creates a new configuration.
