...
| *Script Runs* | Every time Automation scripts are [triggered](xref:Running_Automation_scripts). Among others, it includes Life cycle Service Orchestration (LSO) scripts, Profile-Load Scripts (PLS), Process Automation activities, DOM instance state transitions, and user-defined API calls. | Sum of script runs |
