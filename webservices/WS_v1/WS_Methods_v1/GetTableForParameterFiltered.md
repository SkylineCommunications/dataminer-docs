...
- **SORT**=
    ...
    This filter determines how to sort the filter results. It should be followed by one or more parameter IDs, separated by pipe characters (“\|”). The default order is ascending, but DESC can be added in the filter to apply descending sort order instead. For example, *SORT=12502\|DESC*.     From DataMiner 10.0.0 onwards, you can also specify *SORT=NONE*, to avoid any kind of sorting of the results, which may improve performance.
...
- **FULLFILTER**=
    ...
    This is an expanded form of the VALUE= filter, which can contain brackets and logical operators.     For example:
...
| Item                                | Format                                                                                                        | Description                                                                           |
|-------------------------------------|---------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------|
| GetTableForParameter­FilteredResult | Array of DMAParameter­TableRow (see [DMAParameterTableRow](xref:DMAParameterTableRow)) | The rows of the specified table parameter that match the specified table row filters. |
